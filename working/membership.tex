...
%
\newcommand{\normalform}{free group, double coset normal form}
%
\newcommand{\cA}{{\cal{A}}}
\newcommand{\cD}{{\cal{D}}}
\newcommand{\cF}{{\cal{F}}}
...
$F_2$, amalgamating $H_1$ and $H_2$ via the isomorphism $\phi$: that is $G$ is the group
 with
 presentation 
\begin{equation}\label{eq:Xpresn}
\la X_1\cup X_2 | h_i = h_i', i=1, \ldots ,m\ra.
\end{equation}
...
following process.
\be[Step 1]
\item\label{it:st1} For $i=1,\ldots ,t$, 
...
$g= f_1 \cdots f_t$ is in  reduced form and the process finishes.
\item\label{it:st2}  Now suppose that some $f_i \in H_k$,  and let  $i$ be the
maximal index with this property. Then $\phi^{\pm 1}(f_i)=c\in H_{l}$, $l\neq k$.
If $i = t$, rewrite $g$  in the
form $g = f_1\cdots f_{t-2} f'_{t}$,  where  $f'_{t} = f_{t-1}c$. 
If $1<i < t$, rewrite $g$ as
$g = f_1 \cdots f_{i-2} f'_{i}f_{i+2} \cdots f_t$, where
$f'_{i} = f_{i-1}c f_{i+1}$. If $i=1$, rewrite $g$ as
$g = f'_1 f_{3} \cdots f_t$, where
$f'_{1} = c f_{2}$. In all cases $f'_i\in F_1\cup F_2$. Return to
\ref{it:st1a}.
\ee
...
graph $\G_H\times \G_H$ is constructed. As we pointed out above, $(u',v')\sim
(u,v)$ if and only if there is a path in $\G_H\times \G_H$ from $(u',v')$
to $(u,v)$. Therefore the $\sim$ equivalence class of $(u,v)$ is the
vertex set of the connected component of $\G_H\times \G_H$ containing
$(u,v)$.
...
\ajd{{\large \textbf{This section is significantly different from the previous version, where relative transversals were used.}}}\\[1em]
...
\end{proof}
\ajd{I am leaving this remark where it is, rather than putting it before the lemma above, as I think it is 
easier to follow once the lemma has been established. Are you convinced?}
\begin{remark}
...
$a,b\in H$ and $d\in D$. Then the triple $(a,d,b)$ is called a \emph{\normalform} \emph{of }
$w\in F$ (\emph{with respect to } $D$) if exactly one of \ref{it:d=1}, \ref{it:u_Type1} or  \ref{it:u_Type2}  below holds.
\be
...
%let $N_d=\{(a,d,b): (a,d,b) \textrm{ is a  double
%coset normal form for some} w\in F\}$ 
and let 
$\cT_d=\{b\in H:(a,d,b)\textrm{ is a  \normalform~  for some } w\in F\}$.
\end{definition} 
...
of $H^d$ is accepted by $\G_H$, so $H_d=\{1\}$ and $H$ itself is a transversal for $H_d$; and by definition  
$\cT_d\subseteq H$. 
Moreover, if $d=yez^{-1}$ and $h\in H$ then $yez^{-1}h$ has \normalform~ $(1,yez^{-1},h)$, so $H\subseteq \cT_d$.
...
we must show that if $(a_1,d,b_1)$ and $(a_2,d,b_2)$ are \normalform{s} for elements of $F$, such that $b_2b_1^{-1}\in H_d$,
then $b_1=b_2$. If $d=1$ then $b_1=b_2$, by definition of \normalform, so we may assume that $d$ is in $S$. If 
...
there is some \normalform~ $(a,d,b)$ with $h\in H_db$. Set  
...
We shall compute the \normalform~ of  $yuvz^{-1}$ and then show that it has the required right hand component. 
 Now verify that the \normalform~ of $yuvz^{-1}$ is $(yucy^{-1}, d, zc^{-1}vz^{-1})$ and that 
$h^{-1}zc^{-1}vz^{-1}\in H_d$. ...
 
\end{proof}
\begin{proposition}\label{prop:dctotrans}
Every element of $F$ has a unique \normalform. 
\end{proposition}
\begin{proof}
This follows from Proposition \ref{prop:dcreps}, Lemma \ref{lem:dc_trans} and Lemma \ref{lem:dcrel}.
\end{proof}
~\\[1em]
\ajd{The contents of the previous version of this section, with some examples and lemmas
developed in the interim {\large some of which should perhaps be put back here}, are now in the file scrap.tex}
~\\[1em]
...
${G = F_1 \underset{H_1=H_2}{\ast} F_2}$.  Then $G$ has a
 presentation
\begin{equation}\label{eq:Zpresn}
\la X_1\cup X_2\cup Z  | h_i =z_i= h_i', i=1 \ldots m\ra.
\end{equation}
We refer to  the presentations \eqref{eq:Xpresn}  and  \eqref{eq:Zpresn} as the $X$\emph{-presentation} and the $Z$\emph{-presentation} of $G$,
respectively.

\begin{definition}\label{def:dcnf}
 Assume that $X_1$ and $X_2$ are disjoint and let $D_1$ and $D_2$ be sets of  double coset representatives for
$H_1\le F_1$ and $H_2\le F_2$, respectively. For $j=1,2$ and $d\in D_j$ let 
$\cT_d$ be a right transversal for $H_d=H_j\cap H_j^d$ in $H_j$. 
...
as a word in the amalgamated product with presentation \eqref{eq:Xpresn}. If the reduced form of $g$ is the empty
word then this is also the dc-normal form; with $k=0$ and $h_0=1$. If the reduced form of $g$ is $a\in H_k$, where $k=1$ or $2$, then
$g$ has double coset normal form $\phi_k^{-1}(g)$. 
Otherwise we have $g=f_1\cdots f_r$, where $f_i$ is in $F_{j_i}$ and $f_i\notin H_{j_i}$, for
$j_i\in \{1,2\}$; a word with reduced form of free product length $r$. Assume inductively that 
if $g'\in G\backslash(H_1=H_2)$, and $g'$ has  reduced form $f'_1\cdots f'_t$ of length $t<r$ then $g'$ has a  dc-normal form 
$(h'_0,p'_1,h'_1,\ldots,p'_t,h'_t)$ with $p'_i$ in $D_{k_i}$ if and only if $f'_i\in \FF_{k_i}$   
Then $g'=f_2\cdots f_r$ has  a dc-normal form, 
$(h'_0,p_2,h_2,\ldots,h_{r-1},p_r,h_r)$, with $p_i\in D_{j_i}$. 
Now $f=f_1\phi_{j_1}(h'_0)\in F_{j_1}$ so $f$ has \normalform~ $(a,d,b)$, with $a\in H_{j_1}$, $d\in D_{j_1}$ and $b\in \cT_d$. 
As $f_1\notin H_{j_1}$ and $\phi_{j_1}(h'_0)\in H_{j_1}$,  the element $f$ is not in $H_{j_1}$ so 
the double coset representative $d$ is non-trivial. 
Setting $h_0=a$, $p_1=d$ and $h_1=\phi_{j_1}^{-1}(b)$, we 
have $f_1=_Gh_0p_1h_1$ so 
\begin{align*}
g&=f_1g'=_G f_1(h'_0p_2h_2\cdots h_{r-1}p_rh_r)=_G fp_2h_2\cdots h_{r-1}p_rh_r\\
&=_G h_0p_1h_1p_2h_2\cdots h_{r-1}p_rh_r
\end{align*}
and it follows that $(h_0,p_1,h_1,\ldots ,p_r,h_r)$ is a dc-normal form for $g$. 

\begin{comment}
In this case, let $(a_i,s_i,b_i)$
be the \normalform~ of $f_i$ in $F_{j_i}$ with respect to $D_{j_i}$, where 
$a_i,b_i\in H_{j_i}$ and $s_i\in D_{j_i}$. 

Beginning on the right, with $f_r$, we successively 
rewrite each syllable, %using Lemma \ref{lem:reltran}, and 
moving
elements of $H_1\cup H_2$ to the left. 
In detail, suppose that, for some $p$ such that  $1\le p\le r$ we have $s_{p+1},\ldots ,s_r$ and  
$t_{p+1},\ldots ,t_r$, where $s_i\in D_1\cup D_2$, $t_i\in \cT_{s_i}$, for $i=p+1,\ldots,
r$, and an element $h_p\in H_{j_p}$, such that 
$(h_p,s_{p+1},t_{p+1},\cdots ,s_r,t_r)$ is a double coset normal form for 
 $f_{p+1}\cdots f_r$. 
...
%h_{0}p_1h_{1}p_2 \cdots h_{k-1}p_kh_{{k}}$
\end{comment}


Now suppose that $W=(h_{0},p_1,h_{1},p_2, \ldots, h_{k-1},p_k,h_{{k}})$ and 
$W'=(h_{0}^\prime, p_1^\prime ,h_{1}^\prime , p_2^\prime,
\cdots, h_{k-1}^\prime ,p_k^\prime, h_{{k^\prime}}^\prime)$
are dc-normal forms for $g\in G$. 
%$w$ and $w^\prime$ are words in dc-normal form and that
%$w=_G w^\prime$. 
We show by induction on $k$ that $W=W'$. %We have  
...
let $f_1=\phi_{j_1}(h_0)p_1\phi_{j_1}(h_1)$, where $p_1\in F_{j_1}$. 
 In addition, if $k>0$,  
 for each $i\ge 2$, assuming that $p_i\in F_{j_i}$, 
let $f_i=p_i\phi_{j_i}(h_i)$.
 Then $f_1\cdots f_k$, is
a reduced form, with respect to the presentation \eqref{eq:Xpresn},  for the element $w\in G$. Similarly, starting with $W'$ instead
of $W$, we obtain a reduced
form $f_1^\prime \cdots f_{k^\prime}^\prime$ for $w$,  %$w^\prime=_G w$, 
with
%$f_i'=p'_i\phi_{j'_i}(h'_i)$, $p_i'\in F_{j'_i}$, $i\ge 2$, and 
 the analagous definition of $f'_i$, $i=1,\ldots ,k'$. 
...
Moreover, as $f'_1=_G w\in H_1$ we have 
 $k^\prime =0$ and $f'_1=\phi_1(h_0^\prime)\in H_1$.
As $H_1$ embeds in $G$ this implies that $\phi_1(h_0)=\phi_1(h'_0)$, so
$h_0=h'_0$. Thus the result
...
Moreover, $1=_G w w^{-1}= f_1^\prime \cdots f_{k}^\prime
...
element $w_0$ of $G$. 
Moreover $(h_0,p_1,\ldots, p_{k-1},h_{k-1})$ and $(h'_0,p'_1,\ldots, p'_{k-1},h'_{k-1})$ 
are both dc-normal forms for $w_0$. Hence, from the inductive assumption 
these normal forms are equal; that is   
...
To write words in $\FF(X_1\cup X_2\cup Z)$   in double coset normal form  for
 the group $G$  we run 
an algorithm which combines
the reduction process, outlined on page \pageref{it:st1}, 
 with Algorithm I, as suggested by the proof of the preceeding theorem. 
The following is an outline of this algorithm, which we call 
Algorithm II. More detail is   given in 
 Section 
\ref{sub:summaryII}.\\[1em]

\noindent\textbf{Algorithm II}. \label{alg:alg2} \\

Given a reduced word $w \in \FF(X_1\cup X_2\cup Z)$, if $w$ contains no letters of $(X_1\cup X_2)^{\pm 1}$ then
$w$ has dc-normal form $(w)$ and the algorithm is finished. 
Otherwise  the algorithm begins by
rewriting $w$ as a word in $\FF(X_1\cup X_2)$. This is done from the right: take the maximal subword $w_1$ of
$w$ such that $w=w_0\circ w_1$ with $w_1\in \FF(X_k\cup Z)$, for $k=1$ or $2$. Replace each $Z^{\pm 1}$ letter of $w_1$ 
with its image under $\phi_k$ and freely reduce the result to give a word $u_1\in F_k$. Then $w=_G w_0u_1$. Repeat the process
starting with $w_0$ instead of $w$; and continue until $w_0$ is the empty word. 
\ajd{(If this is too brief there is a longer version, commented out in the latex file)}
\begin{comment}
Initially set $w'$ equal to the empty sequence.

We have $w=w_0xw_1$, where
$x\in (X_k)^{\pm 1}$, for $k=1$ or $2$,   and $w_1\in \FF(Z)$.   If $w_0\in F(X_k\cup Z)$ then 
let $f$ be the word  obtained by replacing each $Z^{\pm 1}$ letter $z$ of $w$ by $\phi_k(z)$ and freely reducing.
Replace  $w'$ with the sequence $(f, w')$ and move on to the next phase of the algorithm, in the next paragraph. On the other hand,
if $w_0$ is not in $F(X_k\cup Z)$ then $w_0=u_0yu_1$, where $y\in X_{3-k}^{\pm 1}$ and  $u_1\in F(X_k\cup Z)$. 
In this case let $f\in \FF(X_k)$ be the    word obtained by replacing each $Z^{\pm 1}$ letter $z$ of $u_1xw_1$ by $\phi_k(z)$ 
and freely reducing. Replace $w'$ with $(f,w')$ and $w$ with $u_0y$. Repeat the procedure of this paragraph (with $k$ replaced by $3-k$, 
$w_1=1$, $x=y$ and 
$w_0=u_0$). 
 
The algorithm has now produced a sequence $w'=(f_1,\ldots ,f_s)$, such that $f_1\cdots f_s$ is in free product normal form and is 
equal in $G$ to $w$.
\end{comment}
The result is a sequence of words $(u_1,\ldots, u_s)$ (where $u_s$ is output first, $u_{s-1}$ second and so on), 
such that $w=_G u_1\cdots u_s$,  $u_i\in F_1\cup F_2$ and  $u_i$ and $u_{i+1}$ do not both
belong to $F_k$, for all $i$ and $k$.
%%%%%%%%%%%%%%%%%%%%%%5

To construct the double coset normal form $W$ from $u=(u_1,\ldots, u_s)$, initially set $W$ equal to an empty sequence. Work through the 
sequence  $u$ from right to left building up $W$ in the process, as follows. Suppose $u_i\in F_{j_i}$. Start with $i=s$.   

Use Algorithm I to find the \normalform~ $(a,d,b)$ of $u_i$.  Set $c=\phi_{1-j_i}\phi_{j_i}^{-1}(a)$. 
If $d\neq 1$, then insert $(d,\phi_{j_i}^{-1}(b))$ at the start (left)  
of $W$. Replace the sequence $u$ with 
$u'=(u_1,\ldots ,u'_{i-1},u'_i,u_{i+1},\ldots ,u_s)$ where $u'_i=db$ and $u'_{i-1}=u_{i-1}c$; so the product of elements of $u$ and $u'$ represent
the same elements of $G$. If $i=1$ then $\phi_{j_i}^{-1}(a)$ is inserted at the start of $W$, and 
the algorithm finishes by outputing $W$. Otherwise, decrease $i$ by $1$ and repeat the procedure of this paragraph (and the next).
  
On the other hand, if $d=1$  then also $b=1$. In this case let $u'_i=u_{i-1}cu_{i+1}$ (taking $u_{-1}$ and 
$u_{s+1}$ to be the empty words, if necessary). Replace $u$ with $u'=(u_1,\ldots ,u_{i-2},u_i',u_{i+2},\ldots,u_s)$ and leave $W$ as it is. 
If $i\neq 1$ then decrease it by $1$. Repeat from the beginning of the previous paragraph; which, given the new value of $i$, 
means starting at the new element $u'_i$. 
(Here $s$ is also decreased, but we leave details to the summary of Section \ref{sub:summaryII}.) 


\begin{example}\label{ex:g}
\ajd{I have not checked this example on this reading: but now there is a program that
will do this, and list the steps as it goes, which I will run later, once I've finished the essential editing.}
...
f_1^\prime&=h^\prime_3h_2^\prime y_1y_2 h_2^\prime h_1^\prime\\
&= z_3z_2 y_1y_2 z_2z_1.
\end{align*}
\end{comment}
After processing the right hand syllable $f_2$, 
  the output list $W$ is  $(x_1^{-1})$ 
...
list $W$ becomes $(y_1y_2, z_2z_1z_3^{-1}z_2^{-1}, x_1^{-1})$ and 
...
\[W=(z_2^2 z_1,  x_1 x_3^3 x_1, z_1^{-1}z_2,
...
produce an inverse automaton $\Delta$; and next
...
again. The result of this  final folding is a new inverse automation $\Delta'$.
The process is repeated, using $\Delta'$ in place of $\Delta$. If at 
some point no changes are made, then $\Delta'$ is the candidate automaton. 

 Suppose this process halts at some stage and that $L$ is the
...
the associated graph $\G_{H_k}$, $k=1,2$. The alphabet of $A_k$ is
...
%
%
If $w$ is an element of $G$ and has normal form $(a_1,\ldots, a_n)$ then, 
in general, the inverse of $w$ need not have normal form 
$(a_n^{-1},\ldots, a_1^{-1})$. Therefore it is convenient to use  generating
sets of subgroups of $G$ that are closed under taking inverses. 
...
if $m_i=0$ then
$h_{i,0}\neq 1$. Assume also that for $1\le i\le s$, there $k_i^{-1}=k_j$, for some $j$. 
Denote
...
until it halts. In general  the loop may repeat infinitely often. However, we shall show that if it does halt at some point then
its output is an 
inverse automaton which accepts  normal forms of elements of $K$. 

First note that in the case where $m_i=0$, for $i=1,\ldots ,s$, we have $K\subseteq H_k$ and an element of $K$ written in
normal form is an element of $\FF(Z)$. Thus a word in normal form represents an element of $K$ if and only if it is accepted 
by $\G_K$. Therefore there is nothing further to do in this case, and from now on we assume that $m_i>0$ for at least one $i$. 
That is, $\G_K$ has at least one edge of type $X_1$ or $X_2$. 
Begin by setting $\D_{(0)}=\G_K$; so  that $\D_{(0)}$ is an inverse automaton, with alphabet $\S$
and $\pi(L(\D_{(0)}))=K$. \\[1em]
...
$\D_{(n)}$ are the same %have the same
 %number of
%$X_1$ and $X_2$ components (see below) 
 then output $\D_{(n+1)}$
and stop. Otherwise, add $1$ to $n$ and  return to   \ref{it:gf1}.
\ee

Algorithm III below describes exactly how to carry out \ref{it:gf1} of
this loop. Since \ref{it:gf1} may run more than once,  the input to 
Algorithm III is assumed to be an arbitrary inverse automaton $\D$,
with alphabet $\S$ and $\pi(L(\D))=K$. The output of \ref{it:gf1} of Algorithm III
is a rooted,
labelled, involutive automaton $\D''$, with alphabet $\S$, which may
...
with $\G_K=\D_{(0)}$,  
 and with an element $w$ of $K$, then for some  $n<\infty$
the Generalised Folding Loop outputs an inverse automaton 
 $\D_{(n)}$ which accepts the
dc-normal form  of $w$. \\[1em]
...
type $X_{k^\prime}$, where $k\neq k^\prime$. The $X_k$ component $\D_{k,0}$ of $\D$ 
is the subgraph of $\D_k$ (and of $\D$)  
 formed by removing all connected components of $\D_k$ which contain no 
edge of type $X_k$: that is by removing isolated vertices or components having edges only of type $Z$. 
 Note that, as $\D$ is connected and has an edge of type $X_1$ or $X_2$, every vertex
and every edge of $\D$ occurs in at least one of $\D_{1,0}$ or $\D_{2,0}$. 

Let $k=1$ or $2$. 
A 
{\em boundary vertex} of $\D_{k,0}$ is defined to be a vertex
$\a$ such that $\a$ is incident, in $\D$, to a vertex which does not
belong to $\D_{k,0}$.
 We shall modify $\D_{k,0}$ so that if $p$ is a path,
from a  boundary vertex $u$  to a boundary vertex $v$ of $\D_{k,0}$,
...
complete the modified versions of $\D_{1,0}$  and $\D_{2,0}$ can be reassembled, by attaching as
they did in $\D$. The modification of $\D_{k,0}$
 consists of
five steps, producing new graphs $\D_{k,1}$, $\D_{k,2}$, $\D_{k,3}$, $\D_{k,4}$
and $\D_{k,5}$. In each case there is a canonical morphism $\th_{k,i}$
from $\D_{k,{i-1}}$ to $\D_{k,i}$ and 
we define the {\em boundary vertices} of $\D_{k,{i}}$ to be the vertices $\th_{k,i}(v)$
of $\D_{k,i}$, such that $v$ is a boundary vertex of $\D_{k,{i-1}}$. Moreover the
 images $\th_{k,i}\circ \cdots\circ \th_{k,1}(v)$ of vertices of $\D_{k,0}$ in $\D_{k,i}$
will be referred to as  {\em (original) vertices of} $\D_{k,0}$.


The modification process is followed by a ``reassembly'' phase, in which
we reconnect  $\D_{1,5}$  and $\D_{2,5}$ 
to form the output of the algorithm. 
To enable the reassembly the algorithm records, when constructing 
$\D_{k,i}$ from $\D_{k,{i-1}}$, the inverse images of vertices of 
$\D_{k,i}$ under the map $\th_{k,i}$. We leave details of the
construction of these inverse image sets to the the summary
in 
Section \ref{sub:summaryIII}. 
 
Fix $k=1$ or $2$ and, to reduce notation,  let $\T_i$ denote the graph $\D_{k,i}$, for $0\le i\le 5$.
\\[1em]

\noindent\textbf{Modification 1: $\T_0\leadsto \T_1$.}\\
This step involves the addition of paths labelled by $X_k$-words corresponding to
$z$-edges of an $X_k$ component.

 If $(\a,z,\b)$ is an
edge of $\T_0$ labelled by an element $z\in Z$ then add a path from
$\a$ to $\b$ labelled by $\phi_k(z)$ to the graph $\T_0$. Do this
...
\end{subfigure}
\end{center}
\caption{$\T_0$ to $\Theta_1$: $w=\phi_k(z)$.}\label{fig:alg2-1}
\end{figure}
Let $\th_{k,1}$ be the composition of the inclusion map from 
$\T_0$ to $\Theta'_1$ with the folding morphism from $\Theta'_1$ to
$\Theta_1$. If $\a$ and $\b$ are vertices of $\Theta_0$ then,
since $\th_{k,1}$ is a morphism
$\pi(L(\Theta_0, \a,\b))\subseteq \pi(L(\Theta_1,\th_{k,1}(\a),\th_{k,1}(\b)))$.
On the other hand if $w$ is in  $L(\Theta_1,\th_{k,1}(\a),\th_{k,1}(\b))$
...
$\a$ to $\b$ in $\Theta_0$, such that $\pi(w^{\prime\prime}) =
\pi(w^\prime)$. Hence $\pi(L(\Theta_1,\th_{k,1}(\a),\th_{k,1}(\b)))
= \pi(L(\Theta_0, \a,\b))$.  \\[1em]
%%%

\noindent\textbf{Modification 2: $\T_1\leadsto \T_2$.}\\
Here we add to $\T_1$ paths labelled by $\phi_k^{-1}$ images of labels of simple
paths in $\T_1\times \G_{H_k}$, from $(\a_i,1)$ to $(\a_j,1)$; for
appropriate $i,j$.
 Let $\cP_1=\T_1\times \G_{H_k}$. Then there is a path
 labelled $w$ from $\a$ to $\b$
in $\T_1$ and a path labelled $w$ from $\a^\prime $
to $\b^\prime$ in $\G_{H_k}$ if and only if there is a path
...
Let $\a_i$, $\a_j$ be vertices of  $\T_0$  with $i< j$. If
\begin{itemize}
\item
 there
exists a simple path $p$ in $\U_1$ from $(\a_i,1)$ to $(\a_j,1)$ and
\item
...
\psfrag{cP}{\hspace*{-1.5em}$\U_1\subseteq \cP_1$}
\psfrag{G_H}{$\G_{H_k}$}
...
$(\a_j,1)$, over all pairs of vertices $\a_i$, $\a_j$ of $\T_0$,
...
give a new graph $\T_2$. The composition $\th_{k,2}$ of the the
...
$\pi(L(\T_1,\a,\b))=\pi(L(\T_2,\th_{k,2}(\a),\th_{k,2}(\b)))$.\\[1em]
...
of $Z$, and all edges of $\G_{H_k}$ are labelled by elements of $X_k$
the graphs $\cP_1=\T_1\times \G_{H_k}$ and $\cP_2=\T_2\times \G_{H_k}$ differ only
...
vertex $(\a,1)$, for some vertex $\a$ of $\T_0$. Let $(\a,1)$ be
...
$p_i,e,p_j$ projects to a closed path in $\G_{H_k}$, based at $1$,
...
previous case there is a natural morphism $\th_{k,3}$ from $\T_2$
...
vertices   $\a$, $\b$ of $\T_2$,
$\pi(L(\T_2,\a,\b))=\pi(L(\T_3, \th_{k,3}(\a),\th_{k,3}(\b)))$.\\[1em]
...
but not accepted, by $\G_{H_k}$.
 Let
$\cP_3=\T_3\times \G_{H_k}$.
...
for some vertices $\a_i$ and $\a_j$ of $\T_0$.
...
\psfrag{Tk in G_H}{$T_k\subseteq \G_{H_k}$}
...
morphism $\th_{k,4}$ from $\T_3$ to $\T_4$. If $L_4$ is the
language accepted by $(\T_4,\th_{k,4}(\a),(\b))$, for some
...
Assume that, for  some vertices $\a_1,\a_2, \b$ of $\T_0$, there exist vertices
...
a natural morphism $\th_{k,5}$ from $\Theta_4$ to $\Theta_5$ and again,
if $\a,\b$ are
vertices of $\Theta_4$ and $L_4$ and $L_5$ are the languages accepted by
$(\Theta_4,\a,\b)$ and $(\Theta_5,\th_{k,4}(\a),\th_{k,4}(\b))$,
...
The modification process is applied to $\D_{1,0}$ and $\D_{2,0}$ to produce 
$\D_{1,5}$ and $\D_{2,5}$, respectively. 
Roughly speaking a new graph is then constructed by
reconnecting $\D_{1,5}$ and $\D_{2,5}$  in the same way as they
were connected in $\D$. In detail, 
define $\D^\prime$ to be the disjoint union of the
 graphs $\D_{1,0}$ and $\D_{2,0}$ and 
 $\D^{\prime}_5$ to be the disjoint union of the graphs 
$\D_{1,5}$ and $\D_{2,5}$. 
 For $k=1$ and $2$, there is 
  a morphism
$\theta_k=\theta_{k,5}\circ\theta_{k,4}\circ\theta_{k,3}\circ\theta_{k,2}\circ\theta_{k,1}$
\label{def:thk} 
from $\D_{k,0}$ to $\D_{k,5}$. Define $\theta$ to be the morphism from
$\D^\prime$ to $\D^\prime_5$ which consists of the union of $\theta_1$ and $\theta_2$. 
 By
construction 
there is an embedding $\nu_k$ of $\D_{k,0}$ into $\D$, for $k=1,2$. Let $\nu$ be the union
of $\nu_1$ and $\nu_2$.

%\noindent\textbf{Output of Algorithm III.}\\
The output of the Reassembly step of Algorithm III is the quotient $\D''$ of
...
\ajd{I'm not happy with this notation $\vim$.}

\begin{lemma}\label{lem:resol-quot}
Let $[\cdot]$ be the  quotient map of the equivalence relation $\approx$ on $V(\D^\prime_5)$
(sending $x$ to its equivalence class $[x]$, for $x  \in V(\D'_5)$). There is a morphism $\rho$ from  $\D$ to
...
\includegraphics[scale=0.5]{rho.eps}
%\includegraphics[scale=0.4]{ex_K.eps}
\caption{The maps  $\rho$ and $\hat\rho$}
\label{fig:rho}
\end{center}
\end{figure}
\begin{proof}
...
%start and final state $1$, such that
%$\pi(L(\D))=K$.
 The result $\D^{\prime\prime}$ of applying  \ref{it:gf1} (i.e. Algorithm III) 
...
\Psi$ is called the \emph{dc-folding morphism}. In particular, the root of $\Phi$ is 
the image $\hat\rho(s)$ of the root $s$ of $\D$. 
(See Figure \ref{fig:rho}.)
\end{definition}
...
start and  final state $\s=\hat\rho(s)$. The   $X_k$  component $\Phi_{k,0}$ of $\Phi$ has no
more connected components than $\D_{k,0}$, for $k=1,2$.
...
$\D^{\prime\prime}$  has  $X_k$ component $\D''_{k,0}$ with no more connected components than $\D_{k,0}$.
...
same component. Let $x'_i$ and $y'_i$ be the initial and terminal vertices of 
$q_i$, $i=1,\ldots ,n$. From Lemma \ref{lem:nfcomp}.\ref{it:nfcomp1},  it
follows that there are paths $p_1,\ldots, p_n$, in $\D^\prime$,
with labels
 $u_1,\ldots , u_n$, such that
$\pi(u_i)=\pi(v_i)$ and
$p_i$ is a path in the component
$C_i=\theta^{-1}(\Xi_i)$ of $\D^\prime$, with 
 initial and terminal vertices of  $x_i$ and  $y_i$ satisfying $\theta(x_i)=x'_i$ and 
$\theta(y_i)=y'_i$. 
...
in Section \ref{sec:TC} below. That is we summarise Algorithms I, II and III
...
\noindent\textbf{Process.}
\be[{B}1]
\item\label{it:alg1_1} Find the maximal prefix $h$ of $w$ accepted by $\G_H$.
Let $w=h\circ f$.
\item If $h=w$ then output $(w,1,1)$ and stop. 
\item Use $\G_H$ to find the maximal $L_Q$-prefix $p$ of $f$.
Let $f=p\circ q$.
\item Find the maximal prefix $g$ of $q^{-1}$ accepted by $\G_H$.
Let $q=r^{-1}\circ g^{-1}$.
\item Use $\G_H$ to find the maximal $L_Q$-prefix $t$ of $r$.
Let $r=t\circ e^{-1}$.
...
\item\label{it:ss} (This step is reached only if $e= 1$.)
 Using $P_0$ and $F_H$ find the representative $(u_0,v_0)$ of $(u,v)$, where
$u=\t(p)$ and $v=\t(t)$.
\item Look up the connecting element $c=c(u,v)$.
\item\label{it:dc2} Let $y=w(u_0)$ and $z=w(v_0)$.
\begin{comment}
\item \ajd{After an initial panic these extra steps were inserted after B12. Now no longer needed. They were to rewrite into the normal form as in 
Lemma \ref{lem:reltran}.}
 Let
$d=yz^{-1}$, let $\b=c^{-1}t^{-1}g^{-1}$ and let $\G_d$ be the component
of $\G_H\times \G_H$ containing $(u_0,v_0)$. Let $T_d$ be the intersection
of the spanning forest $F_H$, for $\G_H\times \G_H$, with $\G_d$ and 
...
\end{comment}
\item Freely reduce $hpcy^{-1}$ 
and $zc^{-1}t^{-1}g^{-1}$ and call the
results $a$ and $b$, respectively.
...
\noindent\textbf{Input.}
\biz
\item $w\in \FF(X_1\cup X_2\cup Z)$.
\eiz
\noindent\textbf{Output.}
\biz
...
\noindent\textbf{Process.}
\be[{C}1]
\item\label{it:alg2_ZtoXstart} If $w\in \FF(Z)$ output $(w)$ and halt. (This assumes $w$ is reduced.)
\item\label{it:alg2_ZtoXnontriv} 
Let $w=a_1^{\e_1}\cdots a_m^{\e_m}$, where $a_i\in X_1\cup X_2\cup Z$ and $\e_i=\pm 1$. 
Let $i$ be maximal such that $a_j\in Z$, for all $j>i$. Let $a_i\in X_k$. 
Set $U$ equal to the empty sequence, $f=k$  and $t=m$.
\item\label{it:alg2_ZtoXsyll}
Find the minimal integer
$j$ such that $0\le j<i$ and $a_s\in Z\cup X_f$, for $j<s\le i$.  
\item\label{it:alg2_ZtoXreplace} Let $v=b_{j+1}^{\e_{j+1}}\cdots b_t^{\e_t}$, where $b_s=a_s$, 
if $a_s\in X_k$, and $b_s=\phi_f(a_s)$, if $a_s\in Z$. Freely reduce $v$ in $F_f$ and then insert
$v$ at the left hand end of the sequence $U$. 
\item\label{it:alg2_ZtoXrepeat} If $j=0$ continue to the next step. Otherwise set $t$ and $i$ equal to $j$
, set  
  $f=3-f$ and repeat from Step \ref{it:alg2_ZtoXsyll}.
\item\label{it:alg2_start} 
Let $U=(f_1,\ldots ,f_s)$. 
%Write $w=f_1\circ \cdots \circ f_s$, where 
%$f_i\in F_1\cup F_2$ and $s$ is as small as possible. 
\item\label{it:C1} Let $L$ be a blank list and  set $i=s$. 
\item\label{it:C2}  
If $f_i\in F_j$ then 
input $f_i$ to the execution phase, Section \ref{sub:alg1ex}, of Algorithm I, for the subgroup
$H_j$ of $F_j$. Let the output be $(a_i,d_i,c_i)$ and 
set $a'_i= \phi_{1-j}\phi_j^{-1}(a_i)$ and $t_i=\phi_j^{-1}(c_i)$.
\item\label{it:C3} 
If $d_i=1$ go to Step \ref{it:C4}. Otherwise 
%If $d_i\neq 1$ then 
append $(d_i,t_i)$ to the left hand end of $L$. 
...
\item\label{it:C4}  If $d_i=1$ (so $t_i=1$) then 
\begin{itemize}
\item if $1<i=s$, replace $f_{i-1}$ with $f_{i-1}a'_i$, delete $f_i$ and subtract $1$ from $s$;
...
\item Stallings automata $\G_{H_1}$ and $\G_{H_2}$ for $H_1$ and $H_2$,
with output labels
...
the following.
\be
\item\label{it:D1a} Remove all edges of type $X_{k'}$, where $k'=3-k$, from $\D$ to give a graph $\D_{k}$.
 \ajd{This messes up the complexity section which refers to $\D'_{k,0}$.}
\item\label{it:D1b} Remove all connected components of $\D_k$ which have no edge
of type $X_k$.  The resulting graph
is $\D_{k,0}$. 
...
to each vertex. (A root of $\cP_k$ must  be chosen:
 ...
 $((v,k), u)$, where
$u$ is minimal in some preassigned order on vertices of $\G_{H_k}$ and
...
\ajd{This part of the algorithm can be made quicker. (The python program uses the quick version.) 
...
form $(\a,1)$, where $\a$ is a vertex of $\D$, do the following.
Let $\cC(\a)$ be the component of $\cP_{k,3}$ containing $(\a,1)$.
For all vertices $(\a_1,\b)$ of $\cC(\a)$, with $\a_1$ a vertex of $\D$ and $\b\neq 1$, let
...
$(\b,\e_1)$ and $(\b,\e_2)$, respectively. Assume that there are boundary vertices
...
\ajd{Probably this step can do less. It is used when the free group, double coset normal form
involves a type 2 representative. So we should make sure that the vertex $\b$ is one at 
which there could be such a representative added. Care needs to be taken, as one half of the 
type 2 representative may be trivial (i.e. it may be $s_1s_2^{-1}$ with $s_2=1$). Close scrutiny of
the proof that the alg works is required to see if anything is possible here.}
...
$\D_{1,5}\cup \D_{2,5}$. 
\item\label{it:D17} (\textbf{Reassembly}.) 
...
$\vim(u,k)\cap \vim(u',k')=\emptyset$, whenever $(u,k)\neq (u',k')$. The resulting graph is denoted $\D''$.
\ee
For each vertex $u$ of $\D'_5$ we have $\vim(u)=\nu(\theta^{-1}(u))$ and it follows that the graph
$\D''$ output by Step \ref{it:D17} is the graph $\D''$ constructed in Section
...
\noindent\textbf{Process.}\\
\be[{E}1]
\item For each $i$ add the normal form of $k_i^{-1}$ to the list $k_1, \ldots ,k_s$, if it is not there already.
...
\item\label{it:loop4} If  $\D_{(n)}$ and
$\D_{(n+1)}$ are the same, output $\D_{(n)}$ and halt. Otherwise,
...
There is one $X_1$ component $\Theta_0$ of this graph, shown in Figure \ref{fig:KX}
and two $X_2$ components, $\Theta^\prime$ and $\Theta^{\prime\prime}$,
shown in Figure \ref{fig:KY}. Thus $\D_{1,0}=\Theta_0$ and 
...
First consider the $X_1$ component $\Theta_0$. Modification 1
...
\caption{$\cP_1=\Theta_1\times \G_{H_1}$}
\label{fig:K_f1-x-g}
\end{center}
\end{figure}
...
\caption{$\cP_4=\Theta_4\times \G_{H_1}$}
\label{fig:K_f4-x-g}
\end{center}
\end{figure}
...
\caption{$\cP_1^\prime=\Theta_1^\prime\times \G_{H_2}$}
\label{fig:K_i1-x-g}
\end{center}
\end{figure}
...
\caption{$\cP_4^\prime=\Theta_4^\prime\times \G_{H_2}$}
\label{fig:K_i4-x-g}
\end{center}
\end{figure}
...
If $w$ is a word in $\FF(X_k\cup Z)$, and $w'$ is the word obtained from $w$ 
by replacing each occurrence of a letter $z \in Z^{\pm}$ by $\phi_k(z)$, and then freely
reducing the result in $\FF_k$, then the double coset normal form of $w$ is by definition
equal to $(h_1,s,h_2)$, where $w'$ has \normalform~ $(u_1,s,u_2)$ and $h_i=\phi_k^{-1}(u_i)$. In the second part of the
next lemma we show that 
if such $w$ is a subword of a word accepted by an inverse automaton $\D$, then the normal form of $w$ is accepted by the 
by the automaton resulting from the operation of the Generalised Folding Loop on $\D$. 

\begin{lemma}\label{lem:nfcomp} Let $\D$ be an inverse automaton with alphabet $\S$ and let 
$\D_{k,0}$ be an $X_k$ component of $\D$, for $k\in \{1,2\}$. 
Let $\a$ and $\b$ be boundary vertices of  $\D_{k,0}$ and let $\th_k$ be the map from
$\D_{k,0}$ to $\D_{k,5}$ defined in the Reassembly step of Section \ref{sec:dca}.  Then   the following hold.
\be
\item\label{it:nfcomp1}
  $\pi(L(\D_{k,0},\a,\b))=\pi(L(\D_{k,5},\th_k(\a),\th_k(\b)))$.
\item\label{it:nfcomp2} Let $w$ be a word
 which is accepted by the automaton $(\D_{k,0}, \a, \b)$. Then the double coset normal form of 
$w$ is $(h_1,s,h_2)$, for some $h_i\in \FF(Z)$, and the word $h_1sh_2$  is accepted by $(\D_{k,5}, \th_k(\a), \th_k(\b))$.
\ee
\end{lemma}
\begin{proof} ~
\ref{it:nfcomp1}.
At each stage of the modification process of Algorithm
III the image under $\pi$ of the language accepted by $(\D_{k,i},\a,\b)$  is
preserved. The result now follows directly from the definition of $\th_k$ on page \pageref{def:thk}.

\ref{it:nfcomp2}.
For $j=1,\ldots ,4$, let $\cP_j=\cP_{k,j}=\D_{k,j}\times \G_{H_k}$ and let $\U_j=\U_{k,j}$ be a spanning tree for $\cP_{k,j}$, as in 
 Algorithm III, Section \ref{sub:summaryIII}. 
 Let $w'$ be the word obtained from $w$ by replacing each occurrence of a letter $z \in Z^{\pm}$ by $\phi_k(z)$, and then freely
reducing the result in $\FF_k$. Then there is a path in  $\D_{k,1}$ from $\a$ to $\b$ with label $w'$. 
Let $w'$ have \normalform~ $(u_1,s, u_2^{-1})$, where  $s\in S_k$, $u_i\in F_k$ and 
$u_2^{-1}\in \cT_s$. Let $h_i=\phi_k^{-1}(u_i)\in \FF(Z)$, so $w=_G w'=_G h_1sh_2^{-1}$.
There are three cases to consider, depending on whether $s=1$ or $s$ is a
representative of type $1$ or type $2$.
 
 First consider the case $s=1$, so $u_2=1$ and $w'=u_1=_G h_1$. Then $w'$ is 
the label of a closed path at $1$ in $\G_{H_k}$ so there is a path $p$ from $(\a,1)$ to $(\b,1)$ in $\cP_1$ 
 with label $w'$. Therefore there is a path from $(\a,1)$ to $(\b,1)$ in $\U_1$ with label $u$, say, 
and during Modification 2 a path with label $h=\phi_k^{-1}(u)$, from $\a$ to $\b$, is added to $\D_{k,1}$. 
If $p$ is in $\U_1$ then $u=w'$ and so the result holds in this case. Suppose then that 
$p$ has $m$ edges not in $\U_1$. Then $p=p_0ep_1$, where $p_0$ is a path in $\U_1$ and $e$ is an edge not in $U_1$, with
label $x\in X^{\pm 1}$.
Suppose that $e=(\g,x,\d)$, where $\g$ and $\d$ are vertices of $\cP_1$, and let $w_0$ and $w_1$ be the labels of $p_0$ and $p_1$,
 respectively. Let $v$ be the label of the path $l$ from $(\a,1)$ to $\d$ in $\U_1$. Then a closed path at $\a$, 
with label $\phi_k^{-1}(w_0xv^{-1})$, 
is added to $\D_{k,2}$ during Modification 3. The path $lp_1$ from $(\a,1)$ to $(\b,1)$ has label $vw_1$ and has 
fewer edges outside $\U_1$ than $p$; so by induction there
is a path from $\a$ to $\b$ in $\D_{k_3}$ with label $\phi_k^{-1}(vw_1)$. As $\D_{k,3}$ is folded 
it follows that it contains a path with label $\phi_k^{-1}(w_0xw_1)=\phi_k^{-1}(w_0xv^{-1})\phi_k^{-1}(vw_1)$, that is with label
$\phi_k^{-1}(w')$. This completes the proof in the case $s=1$. 

Next consider the case where $s$ is of type $1$, say
 $s= a_1 e a_2^{-1}$, where
$a_1$ is a maximal $L_{Q_k}$-prefix and an $L_{T_k}$-prefix of $s$ and
 ...
 Then, as the normal form is the 
...
$a_i=w(\t(b_i))$ and $u_i=g_ib_ia_i^{-1} = \phi_k(h_i)$, $i=1,2$.

Recall that we refer to the images of vertices of $\D_{k,0}$ in $\D_{k,i}$ as
 ``vertices of $\D$'', for $i=1,\ldots ,5$.
Since $g_i\in H_k$ and $w'$ is accepted by $\D_{k,1}$ there is a path
labelled $g_1$ from $\a$ to a vertex $\a_1$ of $\D_{k,0}$ and a path
labelled $g_2$ from $\b$ to a vertex $\b_1$ of $\D_{k,0}$. 
 As in the case where $s=1$,  in $\D_{k,3}$ there exist paths from $\a$ to $\a_1$ and from $\b$ to $\b_1$, with labels
$\phi_k^{-1}(g_1)$ and $\phi_k^{-1}(g_2)$, respectively.
...
label of a closed path in $\D_{k,3}$, based at  $\a$. (All such paths
were added in the construction of $\D_{k,3}$ from $\D_{k,2}$.)
Also, the path $L_{l+1}$, from $(\a,1)$ to $(\a_1,1)$, has
label $v_{l+1}\in H$,
and by construction of $\D_{k,2}$ there is a path from $\a$ to
$\a_1$ in $\D_{k,2}$ with label $\phi_k^{-1}(v_{l+1})$.
The path
...
in $\D_{k,3}$ and $\D_{k,3}$ is folded this means  that $\phi_k^{-1}(g_1)$ is the
label of a path, from $\a$ to   $\a_1$, in $\D_{k,3}$. Similarly,
$\phi_k^{-1}(g_2)$
is the label of a path from $\b$ to $\b_1$ in $\D_{k,3}$.
\end{comment}

The word $b_1$ is readable by $\G_{H_k}$ and there
is a path with label $b_1$ in $\D_{k,1}$ from $\a_1$ to some vertex $\a_2$
of $\D_{k,0}$.
...
$(\a,1)$ to $(\a_2,\e_1)$ (see Figure \ref{fig:nf-1}). By construction $\D_{k,4}$ contains a path
from $\a$ to $\a_2$ with label $h_1^\prime a_1$, where
$h_1^\prime =\phi_k^{-1}(t_1 a_1^{-1})\in \FF(Z)$.
(If $\cP_3$ covers the pair $(\a_1,1)$, $(\a_2,\e_1)$ then
$t_1=a_1$ and $h_1^\prime$ is the empty word, while there
exists a path with label $a_1$ from $\a$ to $\a_2$ in $\D_{k,3}$.)
%Furthermore we have
%$\pi(\phi_k(h_1^\prime)b_1^{\prime\prime})=\pi(t_1)$.
Now $g_1b_1t_1^{-1}\in H_k$ and is the label of a closed
path  in $\cP_1\subseteq \cP_3$ based at $(\a,1)$; so by the
previous part of proof, $\D_{k,3}$ contains a closed path, based at $\a$,
 with label
$w_1^\prime=\phi_k^{-1}(g_1b_1t_1^{-1})$. Hence, as it is folded,
$\D_{k,4}$ contains a path, from $\a$ to $\a_2$,
with label the reduced word obtained from the product
$w_1^{\prime} h_1^\prime a_1$, that is
 $\phi_k^{-1}(g_1b_1a_1^{-1}) a_1=h_1a_1$.
 Similarly, $\D_{k,1}$ contains a path labelled $b_2$ from $\b_1$ to
some vertex $\b_2$ of $\D_{k,0}$, and $\D_{k,4}$ contains a path from
$\b$ to $\b_2$ with label $h_2a_2$.

As $w'$ is the label of a path in $\D_{k,1}$ there is a path from $\a_2$ to
$\b_2$ in $\D_{k,4}$ with label $e$. Therefore, in this case, $\D_{k,4}$ contains
a path from $\a$ to $\b$ which has label $h_1a_1ea_2^{-1}h_2^{-1}=h_1sh_2^{-1}$, as required. 





Finally consider the case where $s$ is a double coset
representative of type $2$, so $s=a_1a_2^{-1}$, where   
 $a_1$ and $a_2$ are labels
of simple paths in the subtree $T_k$. Then   
 there are words
$g_1, g_2, b_1, b_2, c$  in $\FF(X_k)$,  such that
$w=g_1\circ b_1 \circ b_2^{-1}\circ g_2^{-1}$,
$g_i\in H_k$, $b_1$ is a maximal $L_Q$-prefix of
$b_1 \circ b_2^{-1}\circ g_2^{-1}$, $b_2$ is in
$L_Q$, $c=c(\t(b_1),\t(b_2))$, $a_i=w(v_i)$, where
$(v_1,v_2)$ is the $\sim$ representative of $(\t(b_1),\t(b_2))$, 
and $h_i=\phi_k^{-1}(g_ib_ica_i^{-1})$. 

Then $\D_{k,0}$ contains vertices $\g_1=\a,\g_2=\b, \a_1, \a_2$ and $\g$ such that  there are paths
in $\D_{k,1}$ 
labelled $g_i$, from $\g_i$ to $\a_i$, and labelled $b_i$ from $\a_i$ to $\g$,
$i=1,2$. 
Let $\e_i=\t(b_i)$, $i=1,2$. 
As in the previous case,  there is a path labelled 
$b_i$ in $\cP_{k,3}$ and 
 a  path labelled $t_i$  in $\U_3$, from $(\a_i,1)$ to $(\g,\e_i)$. 
Thus, as before,  there is a closed path based at $\g_i$ in $\D_{k,3}$ with
label $w'_i=\phi_k^{-1}(g_ib_it_i^{-1})$, $i=1,2$. (See Figure \ref{fig:nf-2}.) 
\begin{figure}
\begin{center}
\psfrag{a}{$\a=\g_1$}
\psfrag{b}{$\b=\g_2$}
...
\label{fig:nf-2}
\end{figure}
By construction of $\D_{k,5}$ there is also a path labelled
$r=\phi_k^{-1}(t_1ca_1^{-1})a_1a_2^{-1}\phi_k^{-1}(a_2c^{-1}t_2^{-1})$
from $\a_1$ to $\a_2$.
 Therefore, as $\D_{k,5}$ is folded, it contains a path from $\a=\g_1$ to $\b=\g_2$ with label 
...
$\Theta_0$ of $\D^\prime$. As $\pi(L(\Theta_0,x,y))=\pi(L(\Theta_5, \a,\b)
=\pi(L(\Theta_5,\a,\a))$, there is a path in $\T_0$, from $x$ to 
$y$,
with label $w$, such that $\pi(w)=1$. $\T_0$ is embedded in $\D$ by the
...
\ajd{This should become part of Lemma \ref{lem:nfcomp} (or a Corollary following it). It used
to contain a statement about the number of connected components of $(\D_{(n)})_{k,0}$ (never increasing)
which may still be of some use.}  
...
%\item\label{it:loopstop2} 
Let $\D_{(n)}$ be the output from $n$ iterations of the Generalised 
Folding Loop and let $w$ be a word
accepted by $\D_{(n)}$. If $w=a\circ b \circ c$,
where $b$ is a maximal $X_k$ factor of $w$ then $\D_{(n+1)}$ accepts
...
Loop must halt.
\noindent \ref{it:loopstop2}. 
\end{comment}
The word $b$ is accepted by an $X_k$
component of $\D_{(n)}$. By Lemma \ref{lem:nfcomp}.\ref{it:nfcomp2}, 
 the word $v$ is accepted by
$\D_{(n+1)}$, and hence $avc$ is accepted by $\D_{(n+1)}$. 
%It follows,
%by induction, that $\D_{(n)}$ accepts $avc$.
\end{proof}
\begin{lemma}\label{lem:nfacc}
Let $\D_{(0)}$ be an inverse automaton with alphabet $\S$ and let 
$w$ be a word accepted by $\D_{(0)}$ and assume that $w$ factorises as  
as $w=w_1\cdots w_m$, $w_i\in \FF(X_{k_i}\cup Z)$, with $k_i\neq
k_{i+1}$. Then the normal form of $w$ is accepted by 
$\D_{(n)}$, for some $0\le n\le 2m$. 
\end{lemma}
\begin{proof}
Let $w$ be a word accepted by $\D_{(0)}$. Then $w$ factorises as  $w$
as $w=w_1\cdots w_m$, where $w_i$ is accepted by an $X_k$
component of $\D_{(0)}$. Thus $w_i\in \FF(X_{k_i}\cup Z)$, with $k_i\neq
k_{i+1}$. If $m=0$, then $w=1$ and the lemma holds. Suppose that $m>0$ and the
lemma holds for all $w'$ which have such factorisations with fewer than $m$ factors.
Then the normal form $f$ of $w_2\cdots w_m$ is accepted by $\D_{(n)}$, for some $n\le 2(m-1)$. 
Thus the word $w_1f$ is accepted by $\D_{(n)}$. 
 Let $v_1$ be the \normalform~ of $w_1$. Then by Lemma \ref{lem:loopstop}, $v_1f$ is accepted by $\D_{(n+1)}$. 
If $v_1\in \FF(Z)$ then the dc-normal form of $w$ is the word obtained from $v_1f$ by free reduction, so the 
result holds in this case. Otherwise, write the dc-normal form $f$ of $w_2\cdots w_n$ as $h_0f'$, where $h_0$ is the maximal
prefix of $f$ which belongs to $\FF(Z)$. Then the word $v_1h_0$ is accepted by an $X_k$ factor of $\D_{(n+1)}$. Let $v_1h_0$ have  \normalform~
$d$.  By definition the dc-normal form of $w$ is $df'$, and  by Lemma \ref{lem:loopstop}, this is accepted by  $\D_{(n+2)}$. 
\ajd{This is a bit sketchy, sorry. Is it comprehensible?}
\begin{comment}
...
\end{proof}
\end{comment}
\ajd{Given an element of $K$, if we can express it as a product of generators of $K$ then we 
can bound the number of iterations of the loop needed to find it's normal form (for what that's worth!).}

\ajd{I left the rest of this section in to remind (me) what we are aiming to 
prove special cases of.}
...
%$%|V\G_{H_k}|$, where $L^{\prime}_{Q_k} \subset L_{Q_k}$ is the
...
$\G_{H_k}$ and the basis $Y_k$ for $H_k$.
...
\ajd{Not needed no mo.}
(Step D18)%\ref{it:D18}) 
The number of vertices of  $\D_Z$ is at most $V$,
