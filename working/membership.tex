...
of $H$ is accepted by $\G_A$, so $H_d=\{1\}$ and $H$ itself is a transversal for $H_d$; and by definition  $\cT_d\subseteq H$. 
...
Assume then that $d$ is of Type 2, say $d=yz^{-1}$ as in Definition \ref{def:free_dc_nf}.\ref{it:u_Type2} above. In fact we only need the fact that  $\cT_d$ is contained in a right transversal for $H_d$ in $H$. That $\cT_d$ is a transversal can then be proved as follows. Let $h\in H$; so
we need to show that there is some normal form $(a,d,b)$ with $h^{-1}b\in H_d$. Set  
$q=zh^{-1}z^{-1}$ and then write $q=\a\circ u\circ v$, where $\a$ is the maximal prefix of $q$ in $H_d$ and $u$ is the maximal prefix of $a^{-1}q$ which is readable in $\G_A\times \G_A$, starting
at $(u_0,v_0)$. Now verify that the normal form of $yuvz^{-1}$ is $(yucy^{-1}, d, zc^{-1}vz^{-1})$ and that 
$h^{-1}zc^{-1}vz^{-1}\in H_d$. 
We can sort this out later if we want to. 
...
$d$ is of Type 1, that is $d\in S^{(1)}$, then no non-trivial element of $H^d$ is accepted by $\G_A$, so $H_d=\{1\}$ and 
...
then the first letter of $t_i^{-1}$ is equal to the first letter of $c_i$ and so is readable, in $\G_A$, from $\t(p_i)$. As
...
$c_i=c_i'x$. As $c_i$ is a path in $\G_A\times \G_A$ with end point $(u_0,v_0)$, it follows that  that $yc_i^{-1}$ is in $L_Q$, 
...
With $d=yz^{-1}$, denote by $\G_d$ the component of $\G_A\times \G_A$ containing $(u_0,v_0)$. We regard $\G_d$ as an automaton
...
$q_z=zqz^{-1}$ are both words in $L(\G_A)$, that is they belong to $H$. Thus $q=q_y^y=q_z^z\in H^y\cap H^z$. On the other
hand if $s\in H^y\cap H^z$ then $s'=ysy^{-1}$ and $s''=zsz^{-1}$ belong to $H$. As $y\in L_Q(\G_A)$ it follows that 
$s$ labels a closed path in $\G_A$, based at $\t(y)=u_0$. Similarly, $s$ labels a closed path in $\G_A$, based at $v_0$. 
...
the first letter of $t_1^{-1}$ is readable from $\t(p_i)$ in $\G_A$, contrary to the definition of $p_i$ as a 
...
\end{center}
\end{figure}
However, this means that $t_2^{-1}\circ g_{222}^{-1}$ is readable in $\G_A$ from $\t(p_2)$ and, by definition of $p_2$,
 hence $t_2^{-1}\circ g_{222}^{-1}=1$. Thus $g_{222}=1$ and $g_1=g_2$, a contradiction.
Therefore $b_1=b_2$ and the proof is complete.
\end{proof}
...
The Stallings folding $\G_{A_i}$ for $H_i$ is shown in Figure \ref{fig:ce}.
\begin{figure}
\begin{center}
\psfrag{a1}{$a|z_1^{-1}$}
\psfrag{a2}{$a|z_3$}
...
\end{center}
\end{figure}
As $H_i$ is normal in $F_i$, the connected components of $\G_{A_i}\times
\G_{A_i}$ are all isomorphic to $\G_{A_i}$. 
