...
\end{lemma}
\begin{proof}
In fact we only need the fact that  $\cT_d$ is contained in a right transversal for $H_d$ in $H$, and 
this is all I'm going to prove here. That $\cT_d$ is a transversal is almost certainly true as well, but 
we can sort this out later if we want to. To see that  $\cT_d$ is contained in a right transversal for $H_d$ in $H$
...
We claim that  a reduced word $q$ in $F$ belongs to $H_{yz^{-1}}$ if and only if $q$ is accepted by $\G_A\times \G_A$,
with base point $(u_0,v_0)$. Let us denote the latter graph (or automaton) with base point $(u_0,v_0)$ by $\G_d$. 
If $q$ is accepted by $\G_{d}$ then $q_y=yqy^{-1}$ and 
...
of $t_2$ cancels in reducing $r^z$ to $r'$. 
