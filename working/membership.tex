...
(with respect to $H$ and $D$, defined above) if $w=F adt$, 
...
Algorithm I in  Sections \ref{sub:sum_algI}, \ref{sub:alg1ex} and 
\ref{sub:alg1ex2} carries out this process. The first part of
...
Section \ref{sub:sum_algI},  following the proof of Proposition 
\ref{prop:dcreps},  takes  a word  $w\in F$ and outputs an triple
$(h,d,h')$, where $h,h'\in H$, $d\in \{1\}\cup S^{(1)}\cup S^{(2)}$
and $w=_F hdh'$. 
The final part of the algorithm, in Section \ref{sub:alg1ex2}, takes
this triple $(h,d,h')$ as input.  The word $h'$ is rewritten, 
as in   
...
$(5,1)$, so $y=a$ and $z=1$, and the  connecting element is  $j=bc$. 
Therefore the double coset representative of $w$ is $s=yz^{-1}=a$, a 
representative of type 2. 
Then $hpjy^{-1}=abca^{-1}\in H$ and $zj^{-1}t^{-1}=c^{-1}b^{-3}\in H$ and we 
can write $w=(abca^{-1})s(c^{-1}b^{-3})$ as an element of $HsH$.
To express $c^{-1}b^{-3}$ as an element of $H_s\cT_s$ we use the
graph $\G'$ with the highligted spanning tree, and 
root vertex $(5,1)$. The maximal prefix accepted by $\G'$ is $q=c^{-1}b^{-1}$
and $c^{-1}b^{-3}=qb^{-2}$. The terminal vertex of $q$ is $(5,3)$ and 
$w(5,3)=j^{-1}=(bc)^{-1}$.
\ajd{It does not look like a coincidence that the connecting element
crops up again here. Probably some simplification of the argument is
possible.} 
 Hence $q=(qj)j^{-1}$, where $qj\in H_s$ and 
$j^{-1}$ is in $\cT_s$; so $c^{-1}b^{-3}=qb^{-2}=(qj)j^{-1}b^{-2}$, 
with $j^{-1}b^{-2}\in\cT_s$.  
Now $s(c^{-1}b^{-3})=s(qj)j^{-1}b^{-2}=(sqjs^{-1})sj^{-1}b^{-2}$.
Hence 
\[w=(hpjy^{-1})s(zj^{-1}t^{-1}g^{-1})=(hpjy^{-1}sqjs^{-1})sj^{-1}b^{-2}.\]
As $hpjy^{-1}sqjs^{-1}=abca^{-1}ac^{-1}b^{-1}bca^{-1}=abca^{-1}$ and 
$j^{-1}b^{-2}=c^{-1}b^{-3}$ the double coset normal form of $w$ is $(abca^{-1}, a, 
c^{-1}b^{-3})$.
\end{example}
\begin{figure}
\begin{center}
...
\ajd{Here we are assuming that the group $H$ is already given by
 a basis $Y$. We should not do this. The input generators $Y$ may not
freely generate $H$ and the output should include a basis $Y'$ for 
$H$, corresponding to the maximal subtree $T$ of $\G_A$.}
...
\end{comp}
\subsubsection{Execution of Algorithm I, Part 1}\label{sub:alg1ex}
\noindent\textbf{Input.}
\biz
...
\end{comp}
\subsubsection{Execution of Algorithm I, Part 2}\label{sub:alg1ex2}
\subsubsection{Execution of Algorithm II}\label{sub:summaryII}
Given $F_1*_{H_1=H_2} F_2$
 the
steps in this Section \ref{sub:sum_algI} are carried out once for
...
of $F_1$ or $F_2$. The following algorithm combines
the reduction process, outlined on page \pageref{it:st1}, with Algorithm I to
write elements in  double coset normal form in $F_1*_{H_1=H_2} F_2$.
