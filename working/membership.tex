...
\begin{proof}
\be 
\item If $g\in \FF(Y)$ then $g\in F$ so $g=ht$,  for some $h\in H_d$ and $t\in \cT_d'$.
As $\FF(Y)=H$ it follows that $t\in \cT_D$, so $\cT_d$ is a transversal for 
$H_d\in \FF(Y)$.
\begin{comment}
...
of $\cF_Z$ and maps the edge of $\G_H\bs T$ corresponding to the generator $y_i$ to the  edge
(labelled) $z_i$ of $\cF_Z$. Then we relabel the edges of $\G_H$ as $x|v$, where $x\in X^{\pm 1}$ is the
original label, and $v=1$ if $\pi$ maps the edge to the vertex of $\cF_Z$ and otherwise $v$ is the 
label of the image of the edge under $\pi$. In this way $\G_H$ becomes a transducer, which on
...
$F$ of all edges in $E_0$ forms a subgraph of  $\G_{H_a}$ which I claim is a forest. The claim
holds because if $C$ is a non-trivial 
reduced closed path in $F$ then it maps to a closed  path $C'$ in $T$ (since its 
edges all have label $x|1$) and, since the map $\rho$ is an immersion, $C'$ is also reduced, a
contradiction. Now add edges to $F$ to form a spanning subtree $T_a$ of $\G_{H_a}$. 
...
for all $z\in Z$. Clearly (and we all know what that means) $\pi\circ rho=\pi_Z\circ \pi_d$, 
...
Thus internal representives of the  transversal $\cT_a$  
arising from $T_a\subseteq \G_{H_a}$ map
to internal representatives of the  Schreier transversal $\cT_a^{Z}$ arising from 
$\pi_a(T_a)\subseteq \G_a^Z$; and a similar statement holds for external representatives.   
Thus $\cT_a$ maps naturally to a Schreier transversal for $\phi^{-1}(H_a)$. The image
 $\phi(\cT_a^Z)$ is a transversal of $H_a$ in $H$, but not Schreier, in general.  
\ajd{That's what I wanted to see. I haven't quite filled in all details, but it seems right. 
