...
Returning to the situation of Section \ref{sub:2cosetrepr}, where  $F=\FF(X)$, $H$ is a finitely generated subgroup of $F$ and $D=\{1\}\cup S^{(1)}\cup
...
Let $H$ be freely generated by a set $Y=\{y_1,\ldots ,y_m\}$, let 
$Z=\{z_1,\ldots, z_m\}$ be a set, disjoint from $F$, and let $\phi:\FF(Z)\maps H$ be
the isorphism induced by $\phi(z_i)=y_i$. 
 Then 
the set $\phi^{-1}(\cT_d)$ is a Schreier transversal for the subgroup 
$\phi^{-1}(H_d)$ of $\FF(Z)$.
\ajd{Today's first attempt at this said that $\cT_d$ was a Schreier transversal
for $H_d$ in $\FF(Y)$. 
This is almost but not quite true. The problem is that when elements
of $\cT_d$ are factorised as elements of the the group $\FF(Y)$ 
the factors  are rewritten in terms of $Y$,
 and in doing this some cancellation may be introduced (between letters of $X$),
and the factors may then no longer elements of $\cT_d'$.}  
...
$H_d\in \FF(Y)$. Now let $s$ be any element of $\cT_d$ and $s=a\circ b$ be 
