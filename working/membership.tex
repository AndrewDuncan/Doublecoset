...
\emph{relative (Schreier) transversal for } $H_d$ in $H$. 
\end{definition}  
\ajd{Lisa, should the definition above contain the word ``Schreier'' and if so 
should it be in brackets?  
...
$H_d$ in $H$.
\begin{comment}
 Now let $s$ be any element of $\cT_d$ and $s=a\circ b$ be 
a cancellation free factorisation of $s$ in the free group $\FF(Y)$ (that
is $a$,$b\in \FF(Y)$ and, as elements of $\FF(Y)$ the length of $s$ is the sum of the lengths of $a$ and 
$b$). Then 
...
\ee
\end{proof}
\ajd{Now some discussion of relative transversal, which is not necessary, but 
I want to see if it works and it fits naturally here.}
...
\ajd{I haven't quite filled in all details, but it looks about right. 
As you say, we don't need this, but I was curious.}
...
below. We shall use the following facts. \ajd{Maybe these are well
known and we can just quote some source where they can be found. Perhaps
1 is too trivial to write as a lemma. Perhaps we don't need 2 atall.}
...
$L_T$. Let $u_i=\t(a_i)$, $i=0,1$. Then there exists
...
an isomorphism; and so  $\pi_1(\G_d', (u_0,u_1))\cong H^{a_1}\cap H^{a_0}\le F$.
As $\G_d'$ is folded it is a subgraph of the cover
$\G_d^*$ of $H^{a_1}\cap H^{a_0}$, and a spanning tree $T_d'$ for 
$\G_d'$ embeds in a spanning tree $T_d^*$ for $\G_d^*$. (We may assume
we have fixed a spanning tree for each component of $\G_H\times \G_H$.)
...
\begin{comment}
Each element $d\in S^{(2)}$ has the form $a_0a_1^{-1}$, where 
$(\t(a_0),\t(a_1))$ belongs to $P_0$ and 
is a vertex of a unique (non-diagonal) component of 
$\G_H\times \G_H$; and each such component contains exactly one 
vertex of $P_0$ (by definition of $P_0$). Hence for each non-tree
component $C$ of $\G_H\times \G_H$ we may assume that we have chosen a
spanning tree $T_C$ for $C$. Moreover, if the vertex of $C$ which 
belongs to  $P_0$ is $(\t(a_0),\t(a_1))$, as above, we may assume
that $T_C$ has been chosen to contain the maximal possible prefix of the
 path $a_1^{-1}$. 
(Note that this path must be simple, as it maps to a simple 
path in $\G_H$.)  Given $d=a_0a_1^{-1}$, call the component $C$, 
with base point $(u_0,u_1)$,
$\G_{H'}$. (Its core is  the Stallings folding of $H^{a_1}\cap H^{a_0}$.)
Let $T'$ be a spanning tree for $\G_{H'}$ constructed as above, to 
contain the maximal readable prefix of $a^{-1}$. 
\end{comment}

Given $w\in F(X)$ such that $w\in H$, set $w'=a_1^{-1}w$ (as a reduced word).  
 Find the maximal
...
This algorithm forms part of Algorithm I in Section \ref{sub:sum_algI}. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Suppose that, for some $p$ such that  $1\le p\le r$ we 
...
\[f_p h_p=a_p s_p b_p h_p=a_p d_ps_pt_p.\]
...
following algorithm; the steps of which are described in detail in Section 
...
using
\ref{it:st1} and \ref{it:st2}, on page \pageref{it:st1} write $w$ in reduced form, 
$w=w_1\ldots w_r$, where $w_i\in F_{j_i}$, $j_i\in \{1,2\}$ and if $r>1$ then, 
for all $i>1$, we have $w_i\notin H_{j_i}$  and $j_{i-1}\neq j_i$.  
\item
For $i=1,\ldots ,r$,  if $w_i\in F_{j_i}$, 
find the double coset representative $p_i \in D_{j_i}$ such that 
$w_i\in H_{j_i}p_iH_{j_i}$. Write $w_i=h_ip_ih_i'$, with 
$h_i,h_i'\in D_{j_i}$. 
\item
Starting on the right rewrite $w_r$ as $h_rp_rt_r$, where 
$t_r\in \cT_{p_r}$ and $h_r\in H_{j_{r-1}}$, as in the proof of 
Theorem \ref{thm:dcnf}. Continuing this way (as in the proof of Theorem
\ref{thm:dcnf}) find $h_0\in H_{j_0}$, $p_i\in D_{j_i}$ and 
$t_i\in \cT_{p_i}$, such that $w=_G h_0 p_1 t_1 \cdots p_rt_r$. 
\item 
For $i=1,\ldots, r$, replace $t_i$ with $v_i=\phi^{-1}_{j_i}(t_i)$ and 
replace $h_0$ with $v_0=\phi^{-1}_{j_0}(h_0)$. 
\item 
Output the  dc-normal form of 
$w= 
v_0p_1v_1 \cdots p_rv_r$. 
\ee
