...
\item\label{it:nfcomp2} Let $w$ be a word
 which is accepted by the automaton $(\D_{k,0}, \a, \b)$. Then the
normal form of $w$ is accepted by $(\D_{k,5}, \th_k(\a), \th_k(\b))$.
\ee
\end{lemma}
\begin{proof} ~
\ref{it:nfcomp1}.
...
We may assume that $w\in \FF(X_k)$ (given the construction of $\D_{k,1}$). 
\ajd{May this not require an additional repetition of the main loop?} 
Let $w$ have \normalform $(u_1,s, u_2^{-1})$, where  $s\in S_k$, $u_i\in F_k$ and 
$u_2^{-1}\in \cT_s$. Let $h_i=\phi_k^{-1}(u_i)\in \FF(Z)$, so $w=_G h_1sh_2^{-1}$.
...
labelled $g_2$ from $\b$ to a vertex $\b_1$ of $\D_{k,0}$. Therefore, in $\cP_1$,
...
$(\a_1,1)$ to $(\a_2,\e_1)$ (see Figure \ref{fig:nf-1}). By construction $\D_{k,4}$ contains a path
...
exists a path with label $a_1$ from $\a_1$ to $\a_2$ in $\D_{k,3}$.)
%Furthermore we have
...
previous part of proof, $\D_{k,3}$ contains a closed path, based at $\a_1$,
 with label
...
Then $\D_{k,0}$ contains vertices $\g_1=\a,\g_2=\b, \a_1, \a_2$ and $\g$ and there are paths
in $\D_{k,1}$ 
labelled $g_i$, from $\g_i$ to $\a_i$, and labelled $b_i$ from $\a_i$ to $\g$,
$i=1,2$. Moreover, as in the first case there are paths in $\D_{k,3}$ labelled
$\phi_k^{-1}(g_i)$
...
As $w$ is readable in $\D_{k,1}$ there is a path labelled 
$b_i$ from $(\a_i,1)$ to $(\g,\e_i)$ in $\cP_{k,4}$, so there
exists a simple path, labelled $t_i$, from $(\a_i,1)$ to $(\g,\e_i)$ in 
$\U_{k,4}$. Then $\D_{k,3}$ contains a path from $\a_i$ to $\a_i$ with 
