...
%corresponding sets for $\T_i$.
 We use the notation $E=E_k = |E_k \D| + |E_Z \D|$, $e = |E_Z \D|$, $V = |V \D|$, and $q_k = |V
...
\D_{k,4}|+2(\overline{Q}''_k+R_k)\cdot(q_k^2-q_k),
\end{equation}
...
The time  $t_{\D^\prime_5}$ required to  reconstruct $\D^{\prime}_5$
is %
%, from \eqref{thetafin}, applied to $\Theta^{(i)}$, for $i=1,\ldots, p_1$,
%and to $\Phi^{(j)}$, for $j=1,\ldots, p_2$,
\begin{equation}\label{eq:D'}
t_{\D^\prime_5}= t_{\D_1}+ t_{\D_2},
\end{equation}
where  $t_{\D_k}, k=1,2$ are bounded by \eqref{thetafin}.
