...
def align_Hgens_with_computed_basis(H,FZ,F):
    ...
    while test==0:
        ...
        if len(Hgens)>n:
            ...
            (H,FZ)=subgroup_input_and_compute(Hname,n,F)
        else:
            test=1
...
def label_with_Zs(H,flower,Hgens,F): 
    ...
    while True:
        if len(Hgens)==len(H.subgroup_free_gens):
            ...
            print("Please enter a free generating set of the correct size %s " %(n,))
            (H,FZ)=subgroup_input_and_compute(Hname,n,F) 
            Hgens=H.subgp_gens
...
def recompute_free_basis(H,FZ,F):
    ...
    while True: #this is a loop that executes till it's broken out of
        ...
        if ac==0:
            ...
            while True: #a loop that will be broken out of once Nielsen reduced gens have been entered
                while True: #this is a(nother) loop that executes till it's broken out of
                    flower=H.flower
                    stall=label_with_Zs(H,flower,Hgens,F) #relabel Stallings folding with given gens/ Z labels - to set up search for required gens
                    output_graph_file(stall,"ex_paper/stall_labl.gv","stall")
                    ...
                    if ec==0 and cn==0:
                        ...
                        if ec==1: #hopefully this never occurs 
                            ...
                            while str(resp)!='i' and str(resp)!='a':
                                ...
                                print("The generators input for ", Hname, " are not Nielsen reduced")
                                resp='i'
                ...
                nr=build_new_labelling(stall,FZ)
                if nr==0:
                    break
                else:
                    print("The new generators input for ", Hname, " are not Nielsen reduced")
                    print("Please enter a Nielsen reduced basis")
                    n=enter_positive_integer("the number of  generators of the subgroup: ")
                    (H,FZ)=subgroup_input_and_compute(Hname,n,F)
                    Hgens=H.subgp_gens
            ###################end of outer loop
...
def subgroup_input_and_compute(Hname,n,F):
...
def construct_required_folding(Hname,Hgens,testfile,F):
    ...
    (H,FZ)=align_Hgens_with_computed_basis(H,FZ,F)
