...
#G.is_element(v)
#G.is_element(w)
H1=subgroup("H1",[h1,h2,h3,h4,h5,h6],["r","u","v","w","x","z"])
print(H1.coherent)
GH=H1.make_flower()
...
print (str(S))
print "}"
NF=Normal_form(S,"t",DB).spit_out_nf()
print("NF a,b,c =", NF[0],NF[1],NF[2])
print("or NF using map into Z's: a, b, c =", NF[3],NF[1],NF[4])

X1=Graph(rooted=True,label="X1")
vlist=X1.vertices
x1=X1.addVertex()
x2=X1.addVertex()
X1.addEdge(vlist[0],x1,'T')
X1.addEdge(vlist[0],x2,'s')
X1.addEdge(vlist[0],vlist[0],'a')
X1.addEdge(vlist[0],vlist[0],'b')
X1.addEdge(vlist[0],vlist[0],'v')
X1.addEdge(vlist[0],vlist[0],'w')
print "digraph X1 {"
print (str(X1))
print "}"
P1=S.product(X1)
