...
\item\label{it:A17} (\textbf{Begin Reassembly}.) For each vertex $\a$ of $\D_1\cup \D_2$,
...
\item\label{it:A18}  For each vertex $\a$ in $\D_Z$, if $\a$ is incident 
to a vertex $\b$ of $\D$, and $\b$ is an element of $\vim(\g)$, for
some vertex $\g$ of $\D_1\cup \D_2$, then delete $\b$ from  the link of $\a$ and
replace it with $\g$. 
\item\label{it:A19} Form $\D''$ from the union of $\D_1$, $\D_2$ and $\D_Z$. 
\item\label{it:A20}  Construct $\D(II)$, the Stallings folding of $\D''$.
\ee
