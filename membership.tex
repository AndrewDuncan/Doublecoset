...
and from $q$ to a vertex of $\cF$ then $q$ is said to be \emph{bi-accessible}.
If all vertices $q\in Q$ are bi-accessible then $A$ is called \emph{trim}. 
If $\S$ is a
subset of a group $G$
...
\in E$ and $(u, \s, v^\prime) \in E$ imply $v=v^\prime$). 
...
As $A$ is inverse, if $w$ is readable by $A$ then there exists a
...
assumed to be an
arbitrary inverse automaton $\D$, with alphabet $Q=(X_1\cup X_2\cup Z)^{\pm 1}$ and  
...
Let $\D$ be an inverse automaton, with alphabet $Q=(X_1\cup X_2\cup Z)^{\pm 1}$ and 
...
an inverse automaton $\D$, with alphabet $Q=(X_1\cup X_2\cup Z))^{\pm 1}$, 
and let $w$ be a word
...
with alphabet $Q=(X_1\cup X_2\cup Z)^{\pm 1}$, 
