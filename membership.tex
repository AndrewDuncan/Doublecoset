...
Let $F$ be the free group on generators $x_1,x_2,x_3$ and $H$ its
subgroup $H=\la
h_1,h_2,h_3\ra$, where  $h_1= x_1^3$, $h_2=x_2x_3x_2^{-1}$ and 
$h_3=x_1x_2x_3\ra$.

The Stallings automata, $A$ for 
with maximal subtrees $T$ highlighted ia shown
in Figure \ref{fig:stall}. 
The set $L_T$ corresponding to the maximal subtree  $T$ is
 $L_T=\{1,x_1,x_1^{-1},x_2,x_3^{-1} \}$.

To find double coset normal form for elements $f_1 =
x_2x_3^2x_2^{-1} x_1^4x_3^3x_1^{-2}x_3^{-1}x_2^{-1}x_1^{-1}$, in  
the notation of Algorithm I, use  
$A$ to read off the maximal acceptable prefix $h=x_2x_3^2x_2^{-1} x_1^3$; and
then the maximal $L_Q$ prefix $p=x_1$ of the remainder. Then 
$q^{-1}=x_1x_2x_3x_1^2x_3^{-3}$, which has maximal acceptable prefix 
$g=x_1x_2x_3$. The maximal $L_Q$-prefix of the remaining part of $q^{-1}$
is then $t=x_1^2$. Thus
\[f_1=h\circ p\circ e\circ t^{-1}\circ q^{-1}\circ g^{-1}=\]
...
\subfigure[Stallings automaton $A_1$ for $H_1$]{
...
\label{fig:stall2}}
\end{center}
\caption{ Stallings automata for Example \ref{ex:f_1f_2}.}\label{fig:stall}
\end{figure}


\begin{example}\label{ex:f_1f_2}
Let $F=F_1$ be the free group on generators $x_1,x_2,x_3$ and $H_1$ its
subgroup $H_1=\la
h_1,h_2,h_3\ra$, where  $h_1= x_1^3$, $h_2=x_2x_3x_2^{-1}$ and 
$h_3=x_1x_2x_3\ra$.
...
The Stallings automata, $A_1$ for $H_1$ and $A_2$ for $H_2$, 
with maximal subtrees $T_1$ and $T_2$ highlighted are shown
in Figure \ref{fig:stall}. 
