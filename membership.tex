...
\begin{proof} Let $R_k$ be defined as above.
% = \max \{ |\phi_k(z_1)|_k, \ldots,
%|\phi_k(z_1)|_k\}$.

{\bf 1. $\T \leadsto \T_1$.} At this stage we add at most $R_k
\cdot |E_Z \T |$ edges of type $X_k$ and at most $(R_k-1)\cdot
|E_Z \T |$ new vertices to obtain $\T'$. It takes time 
$t_1(|E_k \T'|)$ to fold $\T'$ to  $\T_1$. Clearly,
$|E_k\T_1 | \le |E_k\T' | \le |E_k\T | +  R_k \cdot |E_Z \T |$ and
$|V \T_1| \le |V \T' | \le |V \T|+ (R_k-1)\cdot |E_Z \T |$. Hence the
...
correct? Also I think this line could be 
$t_{\T_1} \preceq t_1(|E_k \T_1^\prime |) + O(R_k \cdot |E_Z \T |)
\preceq t_1(R_k|E \T|)$.}
...
\begin{split}
t_{\T'_3} &\preceq O(|V\T|\cdot |E\cP_2\smallsetminus \U_2|\cdot
...
\end{split}
\end{equation}
One can fold this graph in time $t_1(E\T'_3)$. The number of $X_k$
edges stays the same, and

\begin{equation}\label{eztheta3}
\begin{split}
|E_Z\T_3| &\le |E_Z\T'_3|\le |E_Z\T_2|+\\ &+|V\T|\cdot
...
t_1(|E\T'_3|) + t_{\T'_3},
\end{equation}
where $t_{\T'_3}$ given by (\ref{theta'3}).
...
\begin{split}
t_{\T'_4} &\preceq O(|V\T|^2 \cdot(|V \G_{A_k}|-1)(\overline{Q}'_k+R_k -1))\\
&\preceq O(|V\T|^2 \cdot |V \G_{A_k}|\cdot(\overline{Q}'_k+R_k))
\end{split}
\end{equation}
This new graph can be folded in time $t_1(E\T'_4)$. Here
...
t_1(|E\T'_4|) + t_{\T'_4},
\end{equation}
where $t_{\T'_4}$ given by (\ref{theta'4}).
