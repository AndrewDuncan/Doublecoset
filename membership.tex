...
$(\G_A,s_0,\t(w))$. ($L$ is the set of words $w$ readable by $A$ with 
$\t(w)=1$.) 
...
If $w, s, u\in F(X)$ with $w=s\circ u$ then we say that 
\be[(i)]
\item
$s$ is a $A$-{\em readable prefix} of $w$ is $s$ is readable by $A$;  
\item
an % {\em maximal}
$L_T${\em -prefix} of $w$ if $s\in L_T$ and 
\item a {\em maximal} $A$-readable prefix or $L_T$-prefix if no longer
subword of $w$ is an $A$-readable prefix or $L_T$-prefix, respectively. 
\ee
...
A a word $w\in F(X)$ is a {\em representative of type} $1$ if
\[w=s\circ e \circ t^{-1},\]
where $s$ is a maximal $A$-readable prefix and an $L_T$-prefix of $w$, 
and $t$ is a maximal $A$-readable prefix and an 
