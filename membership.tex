...
generated by $\{h_1,\ldots, h_m\}$ and $\{h_1^\prime,\ldots ,h_m^\prime\}$, and
$Z=\{z_1,\ldots, z_m\}$ is a set disjoint from $X_1\cup X_2$. 
The isomorphism from $H_1$ to $H_2$ which maps $h_i$ to $h_i^\prime$ is 
denoted $\phi$ and the isomorphisms from $\FF(Z)$ to $H_1$ and $H_2$ which
map $z_i$ to $h_i$ and $h_i^\prime$ are denoted $\phi_1$ and $\phi_2$, respectively. 
 We define $G$ to be the free product with amalgamation: 
...
\item  $p_i  \in S_1\cup S_2$,  for $i=1,\ldots, k$,  and 
\item if  $p_i\in S_j$ then $p_{i+1}\notin S_j$, $j\in \{1,2\}$.
\ee
\end{definition}
...
\begin{example}\label{ex:g}
Let $g =f_1 f_2 f_1^\prime f_2^{-1}$ in setting  of Examples \ref{ex:f_1} and
Examples \ref{ex:f_2}.
Set $z_i = h_i = h_i^{\prime}$ for $i= 1,2,3$; then
\begin{align*}
...
In Example \ref{ex:f_1} we found the double-coset normal forms of
$f_1=y_1^6 x_1^4 y_1 x_1^4 y_1 x_1^{-2} y_1^{-3} x_1^{-1}$ and 
$f_2= y_1^3x_1^2y_1^3x_1^{-2}y_1^{-1}x_1y_1^{2}x_1^5$, namely
\[f_1 = (h_2^{2}h_1)x_1y_1x_1^4y_1^{-2} (h_2h_1^{-1}h_3^{-1})\in H_1S_1^{(1)}H_1\]
and 
\[f_2=(h_3^{-1}h_2) x_1^{-1}\in H_1S_1^{(2)}H_1.\]
In Example 
Let $f$...
\end{example}
