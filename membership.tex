...
Let $\D'_5$ be the graph consisting of the disjoint union
...
\be
\item
The graph $\D'$ constructed in Section \ref{sec:dca} is equal to 
the graph $\D'$ constructed at the start of Step \ref{it:C17} of 
Section \ref{sub:summaryII}. 
To avoid ambiguity the graph output by Step \ref{it:C19} will be
called $\DII$ here. In the process of constructing $\DII$
from $\D'$, two vertices $u$ and $v$ of $\D'$ are identified
if and only if there is a sequence $u=u_0,\ldots ,u_n=v$, such 
that $\vim(u_{i-1})\cap \vim(u_ i)\neq \emptyset$, $i=1,\ldots , n$. Moreover,
each vertex of $\s constructed by identifying
vertices $u$ and $v$ when the sets $\vim(u)$ and $\vim(v)$ have
non-empty intersection. In this case a new vertex 
 forming unions of sets $\vim$, as described
in Steps \ref{it:C17} and \ref{it:C18} of Algorithm II.
To show that $\D''$  and $\DII$ are isomorphic 
we must verify that the 
quotient of $\D'$ by the equivalence relation $\approx$ is the graph
 To this
end let $\bowtie$ denote the equivalence relation on vertices of $\D'$ 
generated by setting $\a\bowtie \b$ if and only if  $\vim(\a)\cap 
\vim(\b)\neq \nul$.  The vertices of 
The equivalence classes of these two relations will coincide if and only
if $\nu(\theta^{-1}(\a))=\vim(\a)$, for all $\a\in V(\D')$.  
\item
