...
Let $\{w_1,\ldots ,w_n\}$ be a generating set for $H$, where $w_i\in F$. 
The {\em flower automaton} of $H$ (with respect to this generating set)
...
automaton is formed by identifying all the vertices $v_i$ to form the 
root of resulting graph.  

The flower automaton of $H$ depends on the chosen generating set, and is, in general,
non-deterministic (at the root vertex). Stallings \cite{stallings83} 
showed that the folded graph $\G(H)$ obtained by applying foldings to the 
flower automaton of $H$ is  independent of the generating set chosen. Moreover 
$G(H)$ is inversive and 
every reduced word representing an element of $H$ belongs to the language
accepted by $(\G(H),s)$, where $s$ is the root vertex.  
The $(\G(H),s)$ is  called the {\em Stallings automaton} of $H$. 
