...
which may fail to be deterministic (and possibly may not be trim). 
We then show that, if we start as above
...
The output of Algorithm II is the graph $\D^{\prime\prime}$ formed 
from $\D^\prime_5$ as follows. Let $u$ and $v$ be vertices in
...
$\D^\prime_5$. 
\end{itemize}  
The root vertex of $\D^{\prime\prime}$ is the image of the root of $\D$
in $\D^{\prime\prime}$. 

\begin{lemma}
Let $\a$ and $\b$ be boundary vertices of  an $X_k$ component $\T$ of $\D$ 
and let $w$ be a word
...
$a_1$ is a maximal $L_{Q_k}$-prefix and an $L_{T_k}$-prefix of $s$ and 
 $a_2$  is a maximal $L_{Q_k}$-prefix and an $L_{T_k}$-prefix of $a_2\circ c^{-1}$. 
...
$b_1\circ c \circ b_2^{-1}\circ g_2^{-1}$, 
$b_2$ is a maximal $L_Q$-prefix of $b_2\circ c^{-1}$, $c\neq 1$,
...
\psfrag{(a,1)}{$(\a,1)$}
\psfrag{(a1,1)}{$(\a_1,1)$}
\includegraphics[scale=.6]{LeR.eps}
\end{center}
...
%$\D_k$, for $k=1$ and $2$. 
The  graph $\D^{\prime\prime}$ resulting from Algorithm II is said to be
