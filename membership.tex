...
\end{figure}
Repeat this process for all such edges $e$ 
and
...
As in the previos case there is a natural morphism $\theta_3$ from $\T_2$ to
$\T_3$. 
If $\a$, $\b$ are boundary
vertices of $\T_2$,  $L_2$ is the language accepted by
$(\T_2,\a,\b)$ 
and 
$L_3$ is the
...
The automaton $\Psi$ is involutive and folded by definition.
 ...
 As 
$\D$ is connected  so is $\D^{\prime\prime}$ and therefore also $\Psi$.
Hence $\Psi$ is trim and so inversive. The graphs  
$\D$ and $\D^{\prime\prime}$  have the same number of $X_k$ components.
Folding to form $\Psi$ cannot increase this number, so the final statement
of \ref{it:dcfold1} follows. 
%\item
 \\[1em]
\ref{it:dcfold2}. 
