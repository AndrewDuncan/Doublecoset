...
An inversive alphabet $\S$ is a set of a form $\{a_1, \ldots, ,
a_r, a_1^{-1}, \ldots, a_r^{-1}\}$. An automaton $A = (\S, Q, \d,
\mathcal{S},\mathcal{F})$ is {\em inversive} if $\S$ is an
inversive alphabet and for all $p, q \in Q$ and $a \in \S$,
$\d(p,a)=q$ if and only if $\d(q, a^{-1})=p$. Further, an
automaton $A= (\S, Q, \d, s_0)$ is called {\em trim} if it has no
vertices of degree $1$ except maybe the basepoint $s_0$. An
automaton $A$ is {\em deterministic} if $\d(p,a)=q$ and
$\d(p,a)=q'$ imply $q=q'$.
...
$(\S,Q,\d,\mathcal{S},\mathcal{F})$, and in particular if
...
We now recall the notion of standard Stalling folding of an
automaton, which we will use directly, for the preparatory
``cleaning'' of automata described below, and in generalized form
to produce dc-foldings of this automaton (in Section
\ref{sec:foldings}).
...
Let $A$ be a Stallings automaton for $H$ with start state $s_0 =
1$. Fix a spanning tree $T$ for $A$. For each vertex $v$ of $A$
let $w(v)$ denote the label of the path from $s_0=1$ to $v$ in
$T$. We say the word $w$ is {\em readable} by $A$ if, starting in
the start state $1$ with input $w$, the automaton finishes with
the empty word in some arbitrary state. If $w$ is readable by $A$
we denote the final state of $A$ after reading $w$ (starting in
state $1$) by $\t(w)$. If $w$ is readable by $A$ and $\t(w)=1$
...
>>>>>>> 71241d37b9903c54876d1d2aa30e7ca707e5c526
...
each $\sim$ equivalence class. To be explicit we make a particular
choice of transversal for the equivalence classes, as follows. If
...
$w(u')w(v')^{-1}$ is a reduced word. Let $\pp$ be an equivalence
...
$\pp$. Fix one $\sim$ representative for each equivalence class of
$\sim$ and let $Q$ denote the set of  all these $\sim$
representatives (note that by the minimality conditions of the
definition, if $(u,v)$ is a $\sim$ representative then
$w(u)w(v)^{-1}=w(u)\circ w(v)^{-1}$). A word $w\in F(X)$ is
=======
{\ef
It seems like we should specify different cases , depending on
$u_0, v_0$ and $u_1,v_1$ belonging to the same loop $h_i$ or not;
...
and define $S_1=S^{(1)}\cup S^{(2)}$. In order to ensure
uniqueness of the normal form of an element of $F(X)$ we also need
to choose an element $c(u,v)$ of $F(X)$, for each $(u,v)\in P$, of
$P$: $c(u,v)$ is defined to be the label of some fixed choice of
path from $u$ to $u_0$ and $v$ to $v_0$, where $(u_0,v_0)$ is the
$\sim$ representative of $(u,v)$.
...
\psfrag{h11}{$h_1^{\prime}$}

\psfrag{h21}{$h_2^{\prime}$}

\psfrag{h31}{$h_3^{\prime}$}
...
for $H_2$.]{ \includegraphics[scale=.4]{stallh2.eps}
\label{fig:stall2} }
\end{center}
...
Figure \ref{fig:dc-grapha} shows a dc-graph, with root $\a$, where
$x\in X_1$, $y_1,y_2,y_3\in X_2$, $z\in Z$, the only vertex not in
$V_0$ is $\d$, colours of all edges are shown and the colours of
vertices  are $c(\a)=(0,0)$, $c(\b)=(3,2)$, $c(\g)=(0,2)$,
$c(\d)=(0,0)$, $c(\e)=(0,2)$ and $c(\z)=(1,0)$.
\end{example}
\begin{figure}
\begin{center}
\psfrag{a}{$\a$}
...
obtained from $\D$ by $H$-{\em resolution}.
