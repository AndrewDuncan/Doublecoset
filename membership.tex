...
$(u,\s,v) \in E$.  Note that $\G_A$ satisfies the property  that 
\be
\item[\textbf{Aut}] for all $u,v\in V$ and $\s\in \S$, there is at most one
edge labelled $\s$ from $u$ to $v$. 
\ee
Conversely, given a finite, directed, edge labelled graph $\G$
 with vertex set 
$V$, edge set $E$ and edge labels from a set $\S$, which satisfies 
property Aut then, given 
...
and we shall often refer to  graphs as automata, and
...
The automaton $A$
is {\em deterministic}  or {\em folded} if $\d$ if $\d(u,\s)$
 has exactly one element for all $(u,\s)\in \S\times Q$
 (that is, $(u, a, v)
...
$\G$.
It follows that  the folded  graphs of automata are precisely those
...
all graphs of automata, as they may not safisfy Aut.)
...
for some $d\in S$. In fact we describe an algorithm, Algorithm I, which
rewrites a given word $w$ in this form. In the following proof each
step of Algorithm I is justified as it is introduced. A concise 
description of the algorithm is given later, in Section \ref{sub:sum_algI}.
...
and stop. Otherwise, add $1$ to $n$ and  repeat  \ref{it:gf1}.
\ee
...
%%%%%%%%
\item A map $\phi_k:Z\maps F_k$  inducing an isomorphism from $F(Z)$ to
...
total time required to run the Loop on the input $\D_{(n)}$ is
...
t$, and $\e_i = k$ if $w_i \in F_k$, $k = 1,2$; $t_1$ defined in (\ref{t1}), $W = W_Z +W_1 +W_2$, $M = m_1 + \ldots + m_s$, and $t_{\Loop}(\D_{(n)})$ estimated in \eqref{tloop}, with $\D_{(n)}$ corresponding to the $n-$th iterate of the Loop.
\end{theorem}
\begin{proof}
...
All these structures form precisely the input of Algorithm II. Having $\D_{(0)} = \G_K$, we run the Loop once, and obtain $\D_{(1)}$ in time at most $t_{\Loop}(\D_{(0)})$, as in sections \ref{sub:resolution} and \ref{sub:loop}. 
Notice that $\G_K$ has at
most $M$ components of type $X_1$ and $X_2$.
 The Loop is repeated
 only if $\D_{(n+1)}$ has fewer  $X_1$ and $X_2$ components
than $\D_{(n)}$: so there can be at most $M$ iterations of the loop. Therefore, we can construct an inverse automaton 
