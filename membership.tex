...
\item\label{it:A17} (\textbf{Begin Reassembly}.) For each vertex $(u,k)$ of $\D_{1,5}\cup \D_{2,5}$,
...
\item\label{it:A19} Form $\D''$ from the union of $\D'_5$ and $\D_Z'$:
...
{\bf The total complexity of modifications of $\D_k$.} The bounds on the number of
edges and vertices of the graphs constructed in Modifications 1--5 is non-decreasing, so the
total complexity $t_{\D_k}$ of all modifications of $\D_k$ can be estimated as
...
Hence the time required for Step  \ref{it:A17} is at most
...
(Step \ref{it:A18}.) The number of vertices of  $\D_Z$ is at most $V$,
