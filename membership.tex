...
to form the output of the algorithm. Let $\T$ be an $X_k$ component of $\D_k$.
\\[1em]
...
If, for some vertices $\a_i,\a_j$ of $\T_4$ and $\b$ of $\G_{A_k}$,  
 there
...
\ajd{
\noindent\textbf{Modification 5 revisited: $\T_4\leadsto \T_5$.}\\
Finally we wish to add paths which allow double coset representatives
of type $2$ to be read.
Let $\cP_4=\T_4\times \G_{A_k}$ and choose a spanning forest $\U_4$ for
$\cP_4$.
...
\includegraphics[scale=.5]{alg2-5.eps}
\end{center}
\caption{$\Theta_4$ to $\Theta_5$ revisited:
...
Construct  $\cP_{k,4}=\D_{k,4}\times \G_{A_k}$ and its spanning
 forest $\U_{k,4}$. 
\item\label{it:C15} For all $\b\in V(\D_{k,4})$ do the following. 
For $(\e,\xi)\in P$,
if both $b_1^{-1}=w(\e)^{-1}$ and $b_2^{-1}=w(\xi)^{-1}$ 
are readable in $\D_{k,4}$,
starting from $\b$, and ending at vertices $\a_1$ and $\a_2$, then
add to $\D_{k,4}$ a path from $\a_1$ to $\a_2$ with label
$\phi_k^{-1}(b_1ca_1^{-1})a_1a_2^{-1}[\phi_k^{-1}(b_2ca_2^{-1})]^{-1}$, where
...
\ajd{
\ref{it:C15} revisited.  For each element $(\e_1,\e_2)\in P$ and
...
$\a_1$ and $\a_2$ of $\D_{k,4}$ such that $\vim(\a_i)\neq \emptyset$ and 
...
\ajd{Last 2 paragraphs revisited. 
In the second case,
let $w$ have normal form $h_1 a_1 a_2^{-1} h_2^{-1} $, where
$h_1$ and $h_2$ are in $\FF(Z)$,  $a_1$ and $a_2$ are labels
of simple paths in the subtree $T_k$ and $s=a_1a_2^{-1}$ is a double coset
...
Then $\T$ contains vertices $\g_1,\g_2, \a_1, \a_2$ and $\b$ and there are paths
in $\T_1$ 
labelled $g_i$, from $\g_i$ to $\a_i$, and labelled $b_i$ from $\a_i$ to $\b$,
...
$b_i$ from $(\a_i,1)$ to $(\b,\e_i)$ in $\cP_{k,4}$, so there
exists a simple path, labelled $t_i$, from $(\a_i,1)$ to $(\b,\e_i)$ in 
