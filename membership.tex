...
t_{\T'_2} \preceq O((V + R_k e)^2\cdot (2r)^{(V +
R_k e)\cdot q +1})
\end{equation}
...
\cdot \frac{|V\T_1|\cdot(|V\T_1|+1)}{2}\le\\
&\le R_k \cdot E +
...
\cdot \frac{|V\T_1|\cdot(|V\T_1|+1)}{2}\le\\ &\le V + R_k \cdot e +
\overline{R}_k \cdot B_{Q_k}\cdot (V^2 + e^2+V\cdot e),
\end{split}
\end{equation}
...
{\bf 3. $\T_2 \leadsto \T_3$.} We construct both $\cP_2=\T_2\times \G_{A_k}$ and a spanning
...
(that is the set of non-isolated vertices). Since $|V\cP_2|=|V\T_2|\cdot q$, then $|V'\cP_2|
...
\begin{split}
t_{\T'_3} &= O(|V\T|\cdot |E\cP_2\smallsetminus \U_2|\cdot (\overline{Q}_k+2 |E \U_2| +1))\preceq\\
&\preceq     O(V \cdot (|V\T_2|\cdot q \cdot r) \cdot
(\overline{Q}_k+|V\T_2|\cdot q))\preceq\\
...
\begin{split}
|E\T_3| &\le |E\T'_3|\le |E\T_2|+ V \cdot
\overline{Q}_k\cdot |V\T_2| q r \le \\
...
\begin{split}
|V\T_3| &\le |V \T'_3|\le |V\T_2|+ V \cdot(\overline{Q}_k-1)\cdot |V\T_2|\cdot rq \le\\
...
{\bf 4. $\T_3 \leadsto \T_4$.} 
We construct $\cP_3=\T_3\times
\G_{A_k}$ and a
...
\begin{split}
|E\T_4| &\le |E\T'_4| \le |E\T_3|+ V^2 \cdot (\overline{Q}'_k +
R_k)\cdot(q-1)<\\
...
&\preceq O(|V \T_4|\cdot log(q |V \T_4|))+O (|E\T'_5| \cdot log^{\ast}(|E\T'_5|))+\\
&+O((V +R_k e)^2\cdot (2r)^{(V + R_k e) q +1})+\\
