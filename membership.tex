...
certainly takes no longer than $O(f(n_2))$ (on input of size $n$).
{\ajd I'm not sure about the sentence above. Does it really make sense? 
$O(f(x))$ bounds a function of $x$ and the variable above changes from 
$n_1$ to $n_2$. Strictly speaking I think what is meant is that if 
$f$ and $g$ are non-decreasing functions then we write $O(f(x))\preceq O(f\circ g(x))$.
Then if $h(x)=O(f(x))$ and  $O(f(x))\preceq O(f\circ g(x))$ 
it is certainly true that $h(x)=O(f\circ g(x))$.}

\subsection{The time complexity of constructing double coset normal forms}\label{sub:doubleCo_nf}
...
{\ajd Probably the first part of this section should be written without
the subscript $k$, which is an unecessary complication here. Do we need to worry
...
(observe that we spent  time $O(N^2_k)$ on their
construction), we apply Algorithm I to $w \in F_k$. The analysis
of Algorithm I show that it takes time  $O(|w|_k)$  to obtain
a double coset normal form of $w$. Gathering both estimates
together we obtain a bound of $O(N^2_k + |w|_k)$, as required.
\end{proof}
...
$\cP_2=\T_2\times \G_{A_k}$ construction is  $t_3(|V \T_2|,
