...
subgroups of free groups, and in generalized form to produce
...
a terminal vertex of $\G$ then $\theta(t)$ is a terminal vertex of
$\G^\prime$ and {\ef Do you mean final state?}\item  if
...
} {\ajd I think you're right. Would it suffice to give a reference
like Kapovich,Miasnikov ``Stallings foldings and subgroups of free
groups''? I don't know because I've never read this paper, which
is rather long, but it does refer to spanning trees.}

{\ef Since the text has changed and we have cited them already,
now I'm happy without any changes here.}
...
$tz^{-1}$ are trivial. It looks confusing without it.} In
...
with $\G_K=\D_{(0)}$,  the loop halts, at some $n<\infty$;
at which point $\D_{(n+1)}$ is an inverse automaton which accepts the
...
from a  boundary vertex $u$  to a boundary vertex $v$ of $T$, then
the normal form  of  the label $l(p)$ of $p$ is the label of a
path from $u$ to $v$. Throughout the modification process we keep
track of the boundary vertices, so that once modification is
...
vertices of $q$ to the vertex  $\a$ of $\Theta_2$.
(See Figure \ref{fig:alg2-3}.)
\begin{figure}
\begin{center}
...
\end{center}
\caption{$\theta_2$ to $\theta_3$: $h_l=l_ixl_j$, $w_l=\phi_k^{-1}(h_l)$.}\label{fig:alg2-3}
\end{figure}
...
Let $\D$ be  an inverse automaton,
with alphabet $\S$ and start state $s$,
and let $\D^{\prime\prime}$  and
$\Psi$ be the dc-resolution and dc-folding of $\D$,
respectively.
\be
\item \label{it:dcfold1}
$\Psi$ is
is an inverse automaton, with unique start and  final state $\hat\rho(s)$, and
has no more $X_1$ and $X_2$ components than $\D$.
...
%\item
 \\[1em]
\ref{it:dcfold2}.
Let $w$ be a word accepted by $\Psi$. Since $\Psi$ is formed
by a sequence of foldings of $\D^{\prime\prime}$, there is a word
$v$ such that $v$ is accepted by $\D^{\prime\prime}$ and $\pi(v)=\pi(w)$.
Now $v=v_1\cdots v_n$, where each $v_i$ is accepted by a connected
component $\Xi_i$ of $\D^\prime_5$, and no two consecutive components are
accepted by the same component. From Lemma \ref{lem:nfcomp}.\ref{it:nfcomp1} it
follows that there are words $u_1,\ldots , u_n$, such that $\pi(u_i)=\pi(v_i)$
and $u_i$ is accepted by the component $\theta^{-1}(\Xi_i)$ of $\D^\prime$.
Moreover it follows that $u_1\cdots u_n$ is accepted by $\D$ and
$\pi(u_1\cdots u_n)=\pi(v)$. Therefore $\pi(L(\Psi))\subseteq \pi(L(\D))$.
The reverse inclusion follows from the existence of the morphism $\hat\rho$
from $\D$ to $\Psi$.
\begin{comment}
...
\begin{proof}
\ref{it:loopstop1}.\\
The number of $X_k$ components is not increased by Steps 1 and 2, so
at some point the loop must halt.
\noindent\ref{it:loopstop2}. The word $b$ is accepted by an $X_k$ component
of $\D_0$. By construction, the word $v$ is accepted by $\D_1$, and hence
by $\D_n$. It follows that $\D_n$ accepts $avc$.
\end{proof}
...
\begin{proof}
Let $w$ be a word
accepted by $\D_0$. Then $w$ factorises as  $w$ as $w=w_1\cdots w_n$,
where $w_i$ is accepted by and $X_k$ component of $\D_0$. Thus
$w_i\in F(X_{k_i}\cup Z)$, with $k_i\neq k_{i+1}$. If $n=0$, then
$w=1$ and the lemma holds. Suppose the lemma holds for all $w$ such
that $n\le k$, where $k\ge 0$ and now suppose $n=k+1$.
Let $v_i$ be the normal form of $w_i$.
If $v_i\in F(Z)$, for some $i$ then there is a factorisation of
$w$ with at most $k$ factors, and the result follows. Otherwise,
from Lemma \ref{lem:loopstop}.\ref{it:loopstop2},  $\D_n$ accepts $v_1\cdots v_n$, which
is the normal form of $w$.
\begin{comment}
